...
//		buildLog( AwardType.CASH, change, cash, funcName );
	...
	public void recharge( int number ) {
		...
		// 添加奖励记录
		getAwardManager().record( SAwardType.VIP, this.rechargeGold );
		getAwardManager().record( SAwardType.TU, this.rechargeGold );
	...
	private void updateVipLevel() {
		...
		while( rechargeGold >= maxValue ){
			...
			if( v.getBuyPvpCount() != 0 ){
				isDanBuy			= true;
				this.pvpMateBuyCount+= v.getBuyPvpCount();
			}
...
//		}
	...
	public boolean isCanRecharge( int i ) {
		return monthCardFate[i-1] <= DefaultCfg.MONTH_CARD_FATE_MAX;
	}
	
	public void addMonthCardFate( int id ) {
		if( monthCardFate[id-1] < 0 ) 
			monthCardFate[id-1] = DefaultCfg.MONTH_CARD_FATE_ONE;
		else
			monthCardFate[id-1] += DefaultCfg.MONTH_CARD_FATE_ONE ;
		// 如果实在买60 的那么看90的是不是-1
		if( monthCardFate[2] < 0 && id == 2 )
			monthCardFate[2] = 0;
	...
	public boolean handleMonthCardFate( int id ) {
		if( monthCardFate[id-1] <= 0 ) return false;
		return --monthCardFate[id-1] >= 0;
	}
	...
	public int isHaveMonthCard() {
		for( int i = 0; i < monthCardFate.length; i++ )
			if( monthCardFate[i] > 0 ) return 1;
		return 0;
	}
