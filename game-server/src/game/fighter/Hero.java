...
public class Hero extends FighterBase  {
	...
	*/
	...
	public void settlementPropertyToEquip() {
		...
		// 然后 得出现在的
		equHp				= equBar.getOwnValue( EquPropertyType.HP );
		equAttack			= equBar.getOwnValue( getAttackType() == 0 ? EquPropertyType.PHY_ATTACK : EquPropertyType.MAG_ATTACK );
		...
		// 这里特殊加一下 绝对值
		setCritAbs( (short) equBar.getOwnValue( EquPropertyType.CRIT ) );
		setTenacityAbs( (short) equBar.getOwnValue( EquPropertyType.TENACITY ) );
		setHitAbs( (short) equBar.getOwnValue( EquPropertyType.HIT ) );
		setDodgeAbs( (short) equBar.getOwnValue( EquPropertyType.DODGE ) );
	}
