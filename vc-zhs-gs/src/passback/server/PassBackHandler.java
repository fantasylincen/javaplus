...
//		System.out.println( connection.getRemoteAddress() + " 连接关闭" );
	...
	private void onData( INonBlockingConnection con, byte data[] ) throws IOException{
		...
		if( SystemCfg.PLATFORM.equals( "91" ) )
			...
			handle_lb( content );
		else if( SystemCfg.PLATFORM.equals( "PP" ) )
			handle_pp( content );
	...
	private boolean handle_pp( String content ) {
...
//		System.out.println( content );
		...
		int i				= 0;
		String order_id		= getArray( list[i++] );
		String billno		= getArray( list[i++] );
		String account		= getArray( list[i++] );
		String amount		= getArray( list[i++] );
		String status		= getArray( list[i++] );
		String app_id		= getArray( list[i++] );
		String uuid			= getArray( list[i++] );
		String roleid		= getArray( list[i++] );
		String zone			= getArray( list[i++] );
		String sign			= list[i++].replaceAll( "sign=", "" );
		
		try {
//			return PPSdk.payResultNotify( content, order_id, billno, account, amount, status, app_id, uuid, roleid, zone, sign );
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	private String getArray( String list ){
