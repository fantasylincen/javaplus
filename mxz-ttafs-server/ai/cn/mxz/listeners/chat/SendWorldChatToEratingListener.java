...
public class SendWorldChatToEratingListener implements
		Listener<ReceiveWorldChatEvent> {

	TaskQueue tasks = new TaskQueue();

	@Override
	public void onEvent(final ReceiveWorldChatEvent e) {
	...
	private void sendToErating(ReceiveWorldChatEvent e) {
		...
		try {
			...
			ps.put("toRoleId", getIdInteger(""));
			ps.put("toRoleName", "");
			ps.put("message", encode(getMessage(message.getMessage())));
			...
			String string = ConfigProperties.getString("chatMonitorUrl");

			hr = request.sendGet(string, ps);

			String content = hr.getContent();
			new ChatMonitorResponseChecker().check(content);
		} catch (Throwable e1) {
			Debuger.error("发送世界聊天消息到Erating时 出现故障!", e1);
			// throw new RuntimeException(e1);
		}
	}

	private long getGameID(ReceiveWorldChatEvent e) {
		try {
	...
	private String getKey() {
		...
		String text = D.GAME_ID + "" + serverId
				+ ConfigProperties.getString("chatMonitorKey");

		return Util.Secure.md5(text);
	}
