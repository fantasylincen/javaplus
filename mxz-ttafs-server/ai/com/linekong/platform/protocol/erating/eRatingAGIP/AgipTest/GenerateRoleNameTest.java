...
public class GenerateRoleNameTest extends debugTest {
	public static void main(String args[]){
		...
		GenerateRoleNameRequest request=new GenerateRoleNameRequest();
		request.setRoleGender(5000);
		
		debugSet(request.setRoleCount(200001),"setRoleCount");
		
		
		long request_commandId = 0x10003114L;//CMD_GENERATE_ROLE_NAME
		...
		try{
		   ...
		   for(int j=0;j<2;++j)
		   {
			   ...
			   else
			   {
				   ...
				   GenerateRoleNameRespond respond= new GenerateRoleNameRespond();
				   ...
				   else
				   {
					   ...
					   {
						   System.out.println("getRoleCount="+respond.getRoleCount());
						   String[] role_name=respond.getRoleName();
						   for(int i=0;i<role_name.length;++i)
						   {
							   System.out.println(",role_name="+role_name[i]); 
						   }
					   } 
				   }
