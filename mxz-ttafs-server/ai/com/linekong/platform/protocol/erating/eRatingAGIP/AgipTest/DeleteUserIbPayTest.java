...
import com.linekong.platform.protocol.erating.eRatingAGIP.DeleteUserIbPayRequest;
import com.linekong.platform.protocol.erating.eRatingAGIP.DeleteUserIbPayRespond;
import com.linekong.platform.protocol.erating.eRatingAGIP.eRatingProtocol;
public class DeleteUserIbPayTest extends debugTest {
	...
	DeleteUserIbPayRequest request=new DeleteUserIbPayRequest();
	
	BigInteger DetailID=new BigInteger("1380175328",10);
	debugSet(request.setDetailID(DetailID),"setDetailID");
	debugSet(request.setUserID(123),"setUserID");
	long request_commandId = 0x10003714L;//CMD_DELETE_USER_IB_PAY
	...
	try{
	   ...
	   for(int j=0;j<2;++j)
	   {
		   ...
		   else
		   {
			   ...
			   DeleteUserIbPayRespond respond= new DeleteUserIbPayRespond();
