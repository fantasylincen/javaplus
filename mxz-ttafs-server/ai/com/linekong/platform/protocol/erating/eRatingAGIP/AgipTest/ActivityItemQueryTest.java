...
import com.linekong.platform.protocol.erating.eRatingAGIP.ActivityItemQueryReqest;
import com.linekong.platform.protocol.erating.eRatingAGIP.ActivityItemQueryRespond;
...
public class ActivityItemQueryTest extends debugTest {
	public static void main(String args[]){
		...
		ActivityItemQueryReqest request=new ActivityItemQueryReqest();
		debugSet( request.setActivityId(0xffffffffL),"setActivityId" );
		debugSet( request.setRoleId(0xffffffffL),"setRoleId");
		debugSet( request.setUserId(0xffffffffL),"setUserId");
		debugSet( request.setRoleLevel(0xffff),"setRoleLevel");
		long request_commandId = PDUMessage.CMD_ACTIVITY_ITEM_QUERY;
		...
		try{
		   ...
		   for(int j=0;j<2;++j)
		   {
			   ...
			   else
			   {
				   ...
				   ActivityItemQueryRespond respond= new ActivityItemQueryRespond();
				   ...
				   else
				   {
					   System.out.println("getResultCode="+respond.getResultCode());					   					   					   		   
				   	   System.out.println("getItemCount="+respond.getItemCount());
				   	   if( respond.getResultCode()==1)
				   	   {
				   		   ...
				   		   long[] bgnTime=respond.getBgnTime();
				   		   long[] endTime=respond.getEndTime();
				   		   for(int i=0;i<respond.getItemCount();i++)
				   		   {
				   			   System.out.println("activityId["+i+"]="+activityId[i]);
				   			   System.out.println("itemCode["+i+"]="+itemCode[i]);
				   			   System.out.println("itemNum["+i+"]="+itemNum[i]);
				   			   System.out.println("bgnTime["+i+"]="+bgnTime[i]);
				   			   System.out.println("endTime["+i+"]="+endTime[i]);					   
				   		   }
				   	   }
