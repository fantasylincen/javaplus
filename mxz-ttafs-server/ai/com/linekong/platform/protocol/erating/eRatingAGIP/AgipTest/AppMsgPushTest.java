...
import com.linekong.platform.protocol.erating.eRatingAGIP.AppMsgPushRequest;
import com.linekong.platform.protocol.erating.eRatingAGIP.AppMsgPushRespond;
...
public class AppMsgPushTest extends debugTest {
	public static void main(String args[]){
		...
		AppMsgPushRequest request=new AppMsgPushRequest();
		
		
		debugSet(request.setUserID(123456),"setUserID");
		debugSet(request.setClientType(2001),"setClientType");
		debugSet(request.setAppRegistID("127l"),"setAppRegistID");
		
		
		
		long request_commandId = 0x10003311L;//CMD_APP_MSG_PUSH
		...
		try{
		   ...
		   for(int j=0;j<2;++j)
		   {
			   ...
			   else
			   {
				   ...
				   AppMsgPushRespond respond= new AppMsgPushRespond();
