...
public class GwDataReportTest extends debugTest {
	public static void main(String args[]){
		...
		GwDataReportRequest request=new GwDataReportRequest();
		
		
		debugSet(request.setServerID(123456),"setServerID");
		debugSet(request.setDataCount(3),"setDataCount");
		
		 int[] subjectId={3,3,3};
		 int[] subAmount={400,500,600};
		 request.setDataType(subjectId) ;
		 request.setDataValue(subAmount) ;
		
		
		long request_commandId = PDUMessage.CMD_GW_DATA_REPORT;
		...
		try{
		   ...
		   for(int j=0;j<2;++j)
		   {
			   ...
			   else
			   {
				   ...
				   System.out.println("\n*****GwDataReportRespond result******");
				   GwDataReportRespond respond= new GwDataReportRespond();
