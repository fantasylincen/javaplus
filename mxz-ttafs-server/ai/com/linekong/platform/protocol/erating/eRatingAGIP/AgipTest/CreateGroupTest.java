...
import com.linekong.platform.protocol.erating.eRatingAGIP.CreateGroupRequest;
import com.linekong.platform.protocol.erating.eRatingAGIP.CreateGroupRespond;
import com.linekong.platform.protocol.erating.eRatingAGIP.eRatingProtocol;

public class CreateGroupTest extends debugTest{
	public static void main(String args[]){
		...
		debugSet(request.setGroupName("linekong"),"setGroupName");
		debugSet(request.setGroupType((short)0xff),"setGroupType");
		debugSet(request.setRole_ID(0xffffffffL),"setRole_ID");
		
		long request_commandId = PDUMessage.CMD_CREATE_GROUP;
		...
		try{
		   ...
		   for(int j=0;j<2;++j)
		   {
			   ...
			   else
			   {
				   ...
				   System.out.println("\n*****CreateGroupRespond result******");
				   CreateGroupRespond respond= new CreateGroupRespond();
