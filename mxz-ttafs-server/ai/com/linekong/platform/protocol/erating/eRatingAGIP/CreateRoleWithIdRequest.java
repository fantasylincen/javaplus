...
public class CreateRoleWithIdRequest extends eRatingProtocol {	
	private final int bodyLength=4+4+32+1+1+2+4+2+1+1;
	...
	public byte[] getBody()
	{
		...
		b.putUnsignedInt(userID).putUnsignedInt(roleID).put(fixLength(roleName,32))
	...
	public  int analyzeBody(byte[] CreateRoleWithIdRequestBody)
	{	
		if(CreateRoleWithIdRequestBody.length!=bodyLength)
			return -1;
		
		byte[] body = CreateRoleWithIdRequestBody;
