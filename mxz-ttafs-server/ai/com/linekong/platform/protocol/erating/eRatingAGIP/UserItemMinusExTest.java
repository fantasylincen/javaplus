...
import com.linekong.platform.protocol.erating.eRatingAGIP.AgipTest.debugTest;
public class UserItemMinusExTest extends debugTest {
	public static void main(String args[]){
		...
		UserItemMinusExRequest request=new UserItemMinusExRequest();
		long user_id=123;
		long role_id=858;
		long item_count=2;
		debugSet(request.setUserID(user_id),"setUserID");
		debugSet(request.setRoleID(role_id),"setRoleID");
		debugSet(request.setItemCount(item_count),"setItemCount");
		long[] activityID=new long[(int)item_count];
		String[] itemCode=new String[(int)item_count];
		int[] itemNum=new int[(int)item_count];
		for(int i=0; i<item_count;++i)
		{
			activityID[i]=10000+i;
			itemCode[i]="ls_test"+i;
			itemNum[i]=1+i;
		}
		request.setActivityID(activityID);
		request.setItemCode(itemCode);
		request.setItemNum(itemNum);
		long request_commandId = PDUMessage.CMD_USER_ITEM_MINUS_EX;
		...
		try{
		   ...
		   for(int j=0;j<2;++j)
		   {
			   ...
			   else
			   {
				   ...
				   UserItemMinusExRespond respond= new UserItemMinusExRespond();
