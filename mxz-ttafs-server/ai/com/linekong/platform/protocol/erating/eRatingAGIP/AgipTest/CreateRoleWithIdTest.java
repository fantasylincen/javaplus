...
import com.linekong.platform.protocol.erating.eRatingAGIP.CreateRoleWithIdRequest;
import com.linekong.platform.protocol.erating.eRatingAGIP.CreateRoleWithIdRespond;
import com.linekong.platform.protocol.erating.eRatingAGIP.eRatingProtocol;

public class CreateRoleWithIdTest extends debugTest {
	public static void main(String args[]){
		...
		CreateRoleWithIdRequest request=new CreateRoleWithIdRequest();
		
		debugSet(request.setCommunityID((short)0xff),"setCommunityID");
		debugSet(request.setInitialLevel(0xffff),"setInitialLevel");
		debugSet(request.setRoleGender((short)0xff),"setRoleGender");
		debugSet(request.setRoleName("test001"),"setRoleName");
		debugSet(request.setRoleOccupation((short)0xff),"setRoleOccupation");
		debugSet(request.setUserID(0xffffffffL),"setUserID");
		debugSet(request.setroleID(0xffffffffL),"setroleID");
		debugSet(request.setUserIP(192L*256L*256L*256L+168L*256L*256L+41L*256L+125L),"setUserIP");
		debugSet(request.setUserPort(0xffff),"setUserPort");
		
		long request_commandId = PDUMessage.CMD_CREATE_ROLE_WITH_ID;
		...
		try{
		   ...
		   for(int j=0;j<2;++j)
		   {
			   ...
			   else
			   {
				   ...
				   System.out.println("\n*****CreateRoleWithIdRespond result******");
				   CreateRoleWithIdRespond respond= new CreateRoleWithIdRespond();
