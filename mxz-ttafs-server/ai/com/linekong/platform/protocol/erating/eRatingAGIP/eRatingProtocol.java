...
public abstract class eRatingProtocol implements IERatingProtocol {
	protected final short VERSION1=0x10;
	protected final short VERSION2=0x20;
	protected  final BigInteger  maxUint8Value=new BigInteger("ffffffffffffffff",16);
	...
	static byte[] fixLength(String src, int len) {
		byte[] tmp = null;
		try {
		...
		if (tmp.length >= len) {
			return Arrays.copyOfRange(tmp, 0, len);
	...
	static String getStringByLength(IoBuffer buffer, int length) {
		...
		String ret = "";
		try {
			ret = new String(Arrays.copyOfRange(bs, 0, j), "UTF-8");
		} catch (UnsupportedEncodingException e) {
			log.error("", e);
		} finally {
			bs = null;
