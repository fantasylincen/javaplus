...
import com.linekong.platform.protocol.erating.eRatingAGIP.RecordPluginInfoRequest;
import com.linekong.platform.protocol.erating.eRatingAGIP.RecordPluginInfoRespond;
import com.linekong.platform.protocol.erating.eRatingAGIP.eRatingProtocol;

public class RecordPluginInfoTest extends debugTest {
	public static void main(String args[]){
		...
		RecordPluginInfoRequest request=new RecordPluginInfoRequest();
		debugSet(request.setUserID(5000),"setUserID");
		debugSet(request.setUserID(100001),"setUserID");
		debugSet(request.setRoleID(200001),"setRoleID");
		debugSet(request.setTimeStamp(3565),"setTimeStamp");
		debugSet(request.setUserIP(127l*256l*256l*256l+1l),"setUserIP");
		debugSet(request.setMac("123456abcdef"),"setMac");
		debugSet(request.setMessage("123456abcdef"),"setMessage");
		debugSet(request.setType(85555),"setType");
		
		long request_commandId = 0x10003310L;//CMD_RECORD_PLUGIN_INFO
		...
		try{
		   ...
		   for(int j=0;j<2;++j)
		   {
			   ...
			   else
			   {
				   ...
				   RecordPluginInfoRespond respond= new RecordPluginInfoRespond();
