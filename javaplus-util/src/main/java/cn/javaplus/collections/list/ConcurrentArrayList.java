...
/**
 * A Concurrent array similar to the JDK's CopyOnWriteArrayList but saves a
 * fixed array to avoid extra allocations.
 */
public class ConcurrentArrayList<E> extends AbstractCollection<E> {
	...
	public static void main(String[] args) {
		final Collection<Integer> list = new ConcurrentArrayList<Integer>(
				Integer.class);
		for (int i = 0; i < 100; i++) {

			new Thread() {
				public void run() {
					int a = Util.Random.get(0, 2);
					if (a == 1) {
						while (true) {
							list.add(Util.Random.get(0, 9));
						}
					} else if (a == 2) {
						while (true) {
							list.remove(Util.Random.get(0, 9));
						}
					} else {
						while (true) {
							for (Integer i : list) {
