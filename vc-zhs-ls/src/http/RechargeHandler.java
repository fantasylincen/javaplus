...
public class RechargeHandler extends AbstractAsyncHandler {
	...
	protected void doGet(Map<String, Object> p, Response response ) throws IOException {
		...
		try {
			...
			int index 		= ++code;
			String goodsId 	= p.get( desc[index] ) == null ? "" : (p.get( desc[index] ) + "");// * goodsid String Vcvc12 充值的商品ID
			...
			index 			= ++code;
			String tcd 		= p.get( desc[index] ) == null ? "" : (p.get( desc[index] ) + "");// * tcd String 123465 订单在平台上的订单号
