...
public class SendPrizeByColor {

	public String run() {
		return "RUN_FOUNCTION";
	}

	public String sendPrizeByColor(String user, String name, String count, String color) {
		color = color.replaceAll("色", "");
		UserInfo u = GMUtil.getUser(user);
		int c = GMUtil.getCount(count);
		sendPrizeByColor(u, name, c, color);
		return "赠送物品成功:" + user + ":" + name + ":" + count + " : " + color;
	}
	 ...
	 *            颜色 (绿,红,橙...)
	 */
	public void sendPrizeByColor(UserInfo u, String name, int count, String color) {
		int id = 0;
		AwardType type = null;
		int[] args = null;
		if (isInteger(name)) {
	...
	private int getByName(String name, String color) {
		int id = HeroTempletCfg.getByName(name);
		if (id != 0)
			return id;

		id = EquipmentTempletCfg.getByName(name, color);
		if (id != 0)
			return id;
