...
							 static void put(String key, BasicDBObject o, YinShangDto value) {		o.put(key, MongoGen.toObject(value));	 }	 
...
							 static YinShangDto copy(YinShangDto value) {		return new YinShangDto(value);	 }
...
							static DBObject toObject(YinShangDto value) {		if(value == null) 			return null;		return value.toObject();	}	 	static DBObject toObjectYinShangDto(YinShangDto value) {		return value.toObject();	}	 	static BasicDBList toObjectYinShangDto(List<YinShangDto> value) {		BasicDBList o = new BasicDBList();		for (YinShangDto v : value) {			o.add(toObject(v));		}		return o;	}	 	 
		...
		public static YinShangDao getYinShangDao() {			return new YinShangDao(getCollection("YinShang"));		}
		...
		public RoleDtoCursor findByIsCantPlay(boolean isCantPlay) {						BasicDBObject o = new BasicDBObject("isCantPlay", isCantPlay);			return new RoleDtoCursor(collection.find(o));		}
	...
		public static class YinShangDao {			private DBCollection	collection;			public YinShangDao(DBCollection collection) {			this.collection = collection;		}			public void save(YinShangDto u) {			collection.save(u.toObject());		}			public void delete(YinShangDto u) {			delete(u.getId());		}			public void delete(String id) {			collection.remove(key(id));		}			public YinShangDto get(String id) {			DBObject o = collection.findOne(key(id));			if(o == null) {				return null;			}			YinShangDto x = new YinShangDto();			x.fromDBObject(o);			return x;		}			private BasicDBObject key(String id) {			BasicDBObject o = new BasicDBObject();		o.put("_id", id);			return o;		}			public YinShangDtoCursor find() {			return new YinShangDtoCursor(collection.find());		}			public long getCount() {			return collection.count();		}			public YinShangDtoCursor findById(String id) {			collection.ensureIndex("id");			BasicDBObject o = new BasicDBObject("id", id);			return new YinShangDtoCursor(collection.find(o));		}
		/**		 * 模糊查找		 * 比如   pattern = *lyc*01*		 * 匹配  alyc12370121		 * 匹配  x123lycacbb0100 		 */		public YinShangDtoCursor findByIdFuzzy(String id) {			collection.ensureIndex("id");			id = id.replaceAll("\\*", ".*");			id = "^" + id + "$";			BasicDBObject o = new BasicDBObject("id", Pattern.compile(id, Pattern.CASE_INSENSITIVE));			return new YinShangDtoCursor(collection.find(o));		}
		public YinShangDtoCursor findByRoleId(String roleId) {						BasicDBObject o = new BasicDBObject("roleId", roleId);			return new YinShangDtoCursor(collection.find(o));		}
		/**		 * 模糊查找		 * 比如   pattern = *lyc*01*		 * 匹配  alyc12370121		 * 匹配  x123lycacbb0100 		 */		public YinShangDtoCursor findByRoleIdFuzzy(String roleId) {						roleId = roleId.replaceAll("\\*", ".*");			roleId = "^" + roleId + "$";			BasicDBObject o = new BasicDBObject("roleId", Pattern.compile(roleId, Pattern.CASE_INSENSITIVE));			return new YinShangDtoCursor(collection.find(o));		}
		public YinShangDtoCursor findByPassword(String password) {						BasicDBObject o = new BasicDBObject("password", password);			return new YinShangDtoCursor(collection.find(o));		}
		/**		 * 模糊查找		 * 比如   pattern = *lyc*01*		 * 匹配  alyc12370121		 * 匹配  x123lycacbb0100 		 */		public YinShangDtoCursor findByPasswordFuzzy(String password) {						password = password.replaceAll("\\*", ".*");			password = "^" + password + "$";			BasicDBObject o = new BasicDBObject("password", Pattern.compile(password, Pattern.CASE_INSENSITIVE));			return new YinShangDtoCursor(collection.find(o));		}
			public void clear () {			collection.drop();		}			public YinShangDto createDTO() {			return new YinShangDto();		}			public static class YinShangDtoCursor implements Iterator<YinShangDto>, Iterable<YinShangDto>{				private DBCursor	cursor;			private int pageAll;				public YinShangDtoCursor(DBCursor cursor) {				this.cursor = cursor;			}				public boolean hasNext() {				return cursor.hasNext();			}				public YinShangDto next() {				DBObject next = cursor.next();				YinShangDto dto = new YinShangDto();				dto.fromDBObject(next);				return dto;			}				public int getCount() {				return cursor.count();			}				public void skip(int skip) {				cursor.skip(skip);			}						public void limit(int limit) {				cursor.limit(limit);			}						/**			 * 分页, page从1开始 countOfEveryPage必须大于0			 */			public void page(int page, int countOfEveryPage) {				if(countOfEveryPage <= 0) {					throw new RuntimeException("countOfEveryPage must > 0");				}				int count = getCount();				pageAll = count / countOfEveryPage;				if(count % countOfEveryPage != 0) {					pageAll ++;				}								if(page > pageAll)					page = pageAll;								if(page < 1)					page = 1;									int skip = (page - 1) * countOfEveryPage ;				skip(skip);				limit(countOfEveryPage);			}						public int getPageAll() {				return pageAll;			}				public void remove() {				throw new UnImplMethodException();			}				public Iterator<YinShangDto> iterator() {				return this;			}		}	}
		...
		private boolean isCantPlay = false;
...
			isCantPlay = MongoGen.copy(src.isCantPlay);			
		...
		public boolean getIsCantPlay() {			return this.isCantPlay;		}
		...
		public void setIsCantPlay(boolean isCantPlay) {			this.isCantPlay = isCantPlay;		}
		...
		public void setKeyValueForever(MongoMap<String> keyValueForever) {			this.keyValueForever = keyValueForever;		}
			...
			o.put("isCantPlay", MongoGen.toObject(isCantPlay));			
			...
			isCantPlay = getBoolean(o, "isCantPlay");
	...
		public static class YinShangDto implements MongoDto{		private String id = "";
		private String roleId = "";
		private String password = "";
		public YinShangDto() {		}				/**		 * Copy new one		 */		public YinShangDto(YinShangDto src) {			id = MongoGen.copy(src.id);			
			roleId = MongoGen.copy(src.roleId);			
			password = MongoGen.copy(src.password);			
		...
		public void setPassword(String password) {			this.password = password;		}
			...
			o.put("id", MongoGen.toObject(id));			
			o.put("roleId", MongoGen.toObject(roleId));			
			o.put("password", MongoGen.toObject(password));			
...
	static MongoMap<YinShangDto> copy(MongoMap<YinShangDto> map) {		MongoMapImpl<YinShangDto> m = new MongoMapImpl<YinShangDto>();				for (String key : map.keySet()) {			YinShangDto v = map.get(key);			m.put(key, MongoGen.copy(v));		}		return m;	}			static List<YinShangDto> copy(List<YinShangDto> list) {		List<YinShangDto> ls = Lists.newArrayList();		for (YinShangDto t : list) {			ls.add(MongoGen.copy(t));		}		return ls;	}		@Override		public String toString() {			return toObject().toString();		}	}
