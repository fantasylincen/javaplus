...
public class CullTest extends GdxTest implements ApplicationListener {
	...
	public void create() {
		...
		sphere = builder.createSphere(2f, 2f, 2f, 16, 16, new Material(
				new ColorAttribute(ColorAttribute.Diffuse, Color.WHITE)),
				Usage.Position | Usage.Normal);
		// cam = new PerspectiveCamera(45, Gdx.graphics.getWidth(),
		// Gdx.graphics.getHeight());
		cam = new OrthographicCamera(45,
				45 * (Gdx.graphics.getWidth() / (float) Gdx.graphics
						.getHeight()));
		...
		for (int i = 0; i < instances.length; i++) {
			pos.set(rand.nextFloat() * 100 - rand.nextFloat() * 100,
	...
	public void render() {
		...
		for (int i = 0; i < instances.length; i++) {
			...
			if (cam.frustum.sphereInFrustum(pos, 1)) {
				((ColorAttribute) instances[i].materials.get(0).get(
		...
		batch.begin();
		font.draw(batch, "visible: " + visible + "/100" + ", fps: "
				+ Gdx.graphics.getFramesPerSecond(), 0, 20);
		batch.end();
	}
