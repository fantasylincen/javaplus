...
/**
 * For testing and benchmarking of gdx.utils.Select and its associated
 * algorithms/classes
 * 
 * @author Jon Renner
 */
public class SelectTest extends GdxTest {
	...
	public static void performanceTest(int runs, int k) {
		...
		print(msg
				+ String.format(
						...
						allPerf.time.min, allPerf.time.max,
						allPerf.time.average * 1000, allPerf.time.total * 1000,
						comparisonsMade));
	}
	...
	public static void getKthNearestEnemy(boolean silent, int k) {
		...
		if (silent)
			return;
		print(String
				.format("found nearest. min: %.4f, max: %.4f, avg: %.4f, total: %.3f ms",
						perf.time.min * 1000, perf.time.max * 1000,
						perf.time.average * 1000, perf.time.total * 1000));
	}

	public static void verifyCorrectness(Dummy d, int k) {
		enemy.sort(distComp);
		int idx = enemy.indexOf(d, true);
		// remember that k = min value = 0 position in the array, therefore k -
		// 1
		if (enemy.get(idx) != enemy.get(k - 1)) {
			System.out.println("origin dummy: " + originDummy);
			System.out.println("TEST FAILURE: " + "idx: " + idx
					+ " does not equal (k - 1): " + (k - 1));
			throw new GdxRuntimeException("test failed");
		}
	}
	...
	static class Dummy {
		...
		public boolean equals(Object obj) {
			if (!(obj instanceof Dummy)) {
				throw new GdxRuntimeException(
						"do not compare to anything but other Dummy objects");
			}
			Dummy d = (Dummy) obj;
			// we only care about position/distance
			float epsilon = 0.0001f;
			float diff = Math.abs(d.pos.x - this.pos.x)
					+ Math.abs(d.pos.y - this.pos.y);
			if (diff > epsilon)
				return false;
			return true;
