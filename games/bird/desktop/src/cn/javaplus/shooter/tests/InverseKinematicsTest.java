...
public class InverseKinematicsTest extends GdxTest {
	...
	public void create() {
		...
		bones = new Bone[] { new Bone("bone0", 0, 0, 0),
				new Bone("bone1", 0, 2, 2), new Bone("bone2", 0, 4, 2),
	...
	public void render() {
		...
		if (Gdx.input.isTouched())
			camera.unproject(globalCoords.set(Gdx.input.getX(),
					Gdx.input.getY(), 0));
	...
	private void renderBones() {
		...
		for (int i = 0; i < bones.length - 1; i++) {
			renderer.line(bones[i].position.x, bones[i].position.y,
					bones[i + 1].position.x, bones[i + 1].position.y);
		}
		renderer.end();
	...
	public void solveFakeIK(Vector3 target) {
		...
		for (int i = 0; i < bones.length - 1; i++) {
			...
			bones[i + 1].inertia.add((bones[i + 1].position.x - x) * delta,
					(bones[i + 1].position.y - y) * delta, 0).scl(0.99f);
