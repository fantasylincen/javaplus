...
public class EdgeDetectionTest extends GdxTest {
	...
	public void create() {
		...
		/*
		 * shader = new
		 * ShaderProgram(Gdx.files.internal("data/shaders/default.vert"
		 * ).readString(), Gdx.files.internal(
		 * "data/shaders/depthtocolor.frag").readString()); if
		 * (!shader.isCompiled()) { Gdx.app.log("EdgeDetectionTest",
		 * "couldn't compile scene shader: " + shader.getLog()); }
		 */
		batchShader = new ShaderProgram(Gdx.files.internal(
				"data/shaders/batch.vert").readString(), Gdx.files.internal(
				"data/shaders/convolution.frag").readString());
		if (!batchShader.isCompiled()) {
			Gdx.app.log(
					"EdgeDetectionTest",
					"couldn't compile post-processing shader: "
							+ batchShader.getLog());
		}
		...
		fbo = new FrameBuffer(Format.RGB565, Gdx.graphics.getWidth(),
	...
	public void render() {
		...
		batch.draw(fboRegion, 0, 0, Gdx.graphics.getWidth(),
				Gdx.graphics.getHeight());
		batch.end();
		logger.log();
	}
