...
public class NinePatchTest extends GdxTest {
	...
	// Make a new 'pixmapSize' square texture region with 'patchSize' patches in
	// it. Each patch is a different color.
	static TextureRegion newPatchPix(int patchSize, int pixmapSize) {
		...
		for (int x = 0; x < pixmapSize; x += patchSize) {
			for (int y = 0; y < pixmapSize; y += patchSize) {
				p.setColor(x / (float) pixmapSize, y / (float) pixmapSize,
						1.0f, 1.0f);
	...
	// Make a upper-left "quad" patch (only 4 patches defined in the top-left
	// corner of the ninepatch)
	...
	static NinePatch newMidlessPatch() {
		...
		final int pixmapDim = MathUtils.nextPowerOfTwo(Math.max(fullPatchWidth,
				fullPatchHeight));

		Pixmap testPatch = new Pixmap(pixmapDim, pixmapDim,
		...
		for (int x = 0; x < fullPatchWidth; x += patchSize) {
			for (int y = 0; y < fullPatchHeight; y += patchSize) {
				testPatch.setColor(x / (float) fullPatchWidth, y
						/ (float) fullPatchHeight, 1.0f, 1.0f);
				testPatch.fillRectangle(x, y, patchSize, patchSize);
			}
		}

		return new NinePatch(new TextureRegion(new Texture(testPatch),
				fullPatchWidth, fullPatchHeight), patchSize, patchSize,
				patchSize, patchSize);
	}
	...
	public void render() {
		...
		// Dim a np by setting its color. Also test sending same np to batch
		// twice
	...
	public void resize(int width, int height) {
		float ratio = ((float) Gdx.graphics.getWidth() / (float) Gdx.graphics
				.getHeight());
		int h = 10;
		int w = (int) (h * ratio);
