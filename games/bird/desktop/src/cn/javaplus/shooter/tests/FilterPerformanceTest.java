...
public class FilterPerformanceTest extends GdxTest {
	...
	int mode = 0;
	String modeString = "";
	int[] filters = { GL20.GL_NEAREST, GL20.GL_LINEAR,
			GL20.GL_NEAREST_MIPMAP_NEAREST, GL20.GL_LINEAR_MIPMAP_NEAREST,
			GL20.GL_LINEAR_MIPMAP_LINEAR };
	String[] filterNames = { "nearest", "linear", "nearest mipmap nearest",
			"linear mipmap nearest", "linear mipmap linear" };

	void setTextureFilter(int filter) {
		atlas.findRegion("map").getTexture().bind();
		Gdx.gl.glTexParameterf(GL20.GL_TEXTURE_2D, GL20.GL_TEXTURE_MIN_FILTER,
				filters[filter]);
		texture.bind();
		Gdx.gl.glTexParameterf(GL20.GL_TEXTURE_2D, GL20.GL_TEXTURE_MIN_FILTER,
				filters[filter]);
	...
	void setModeString() {
		modeString = (mode % 2 == 0 ? "Sprite" : "Atlas") + " "
				+ filterNames[mode / 2];
	}
	...
	public void create() {
		...
		atlas = new TextureAtlas(Gdx.files.internal("data/issue_pack"),
	...
	public void render() {
		...
		batch.begin();
		font.draw(batch,
				modeString + " fps:" + Gdx.graphics.getFramesPerSecond(), 26,
				65);
		batch.end();
	}
