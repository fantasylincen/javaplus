...
public class Gdx2DTest extends GdxTest {
	...
	Texture textureFromPixmap(Gdx2DPixmap pixmap) {
		Texture texture = new Texture(pixmap.getWidth(), pixmap.getHeight(),
				Format.RGB565);
		texture.bind();
	...
	void drawToPixmap(Gdx2DPixmap pixmap) {
		...
		if (pixmap.getPixel(15, 16) != clearColor)
			throw new RuntimeException("error clear: "
					+ pixmap.getFormatString());
		if (pixmap.getPixel(16, 16) != pixelColor)
			throw new RuntimeException("error pixel: "
					+ pixmap.getFormatString());
		...
		pixmap.fillTriangle(16, 16, 18, 18, 20, 14,
				Color.rgba8888(0, 0.5f, 0, 0.5f));
	...
	public void create() {
		...
		Gdx2DPixmap composite = new Gdx2DPixmap(512, 256,
				Gdx2DPixmap.GDX2D_FORMAT_RGBA8888);
		composite.clear(0);
