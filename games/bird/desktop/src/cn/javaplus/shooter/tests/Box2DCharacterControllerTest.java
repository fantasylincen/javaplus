...
public class Box2DCharacterControllerTest extends GdxTest implements
		ApplicationListener {
	...
	private void createWorld() {
		...
		float y2 = y1;
		for (int i = 0; i < 50; i++) {
			Body ground = createEdge(BodyType.StaticBody, -50 + i * 2, y1, -50
					+ i * 2 + 2, y2, 0);
			y1 = y2;
		...
		for (int i = 0; i < 20; i++) {
			box = createBox(BodyType.DynamicBody, (float) Math.random(),
					(float) Math.random(), 3);
			box.setTransform(
		...
		for (int i = 0; i < 20; i++) {
			Body circle = createCircle(BodyType.DynamicBody,
					(float) Math.random() * 0.5f, 3);
			circle.setTransform(
					(float) Math.random() * 10f - (float) Math.random() * 10f,
					(float) Math.random() * 10 + 6,
					(float) (Math.random() * 2 * Math.PI));
		...
		platforms.add(new MovingPlatform(-2, 3, 2, 0.5f, 2, 0,
				(float) Math.PI / 10f, 4));
	...
	private Body createEdge(BodyType type, float x1, float y1, float x2,
	...
	public void render() {
		...
		if (!grounded) {
			...
			// dampen sudden changes in x/y of a MovingPlatform a little bit,
			// otherwise
			// character hops :)
		...
		// since Box2D 2.2 we need to reset the friction of any existing
		// contacts
		...
		if (jump) {
			...
			if (grounded) {
				...
				System.out
						.println("jump before: " + player.getLinearVelocity());
			...
			if (contact.isTouching()
				...
				if (below) {
					if (contact.getFixtureA().getUserData() != null
						...
						groundedPlatform = (Platform) contact.getFixtureA()
								.getBody().getUserData();
					}

					if (contact.getFixtureB().getUserData() != null
							&& contact.getFixtureB().getUserData().equals("p")) {
						groundedPlatform = (Platform) contact.getFixtureB()
								.getBody().getUserData();
					}
					return true;
				}
	...
	public boolean touchDown(int x, int y, int pointerId, int button) {
		...
		if (button == Input.Buttons.LEFT) {
			if (last == null) {
				...
				createEdge(BodyType.StaticBody, last.x, last.y, point.x,
						point.y, 0);
	...
	class MovingPlatform extends Platform {
		...
		public MovingPlatform(float x, float y, float width, float height,
				float dx, float dy, float da, float maxDist) {
