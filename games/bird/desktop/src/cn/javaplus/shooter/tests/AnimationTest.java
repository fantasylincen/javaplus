...
public class AnimationTest extends GdxTest {
	...
	class Caveman {
		...
		public void update(float deltaTime) {
			stateTime += deltaTime;
			pos.x = pos.x
					+ (headsLeft ? -VELOCITY * deltaTime : VELOCITY * deltaTime);
			if (pos.x < -64)
	...
	public void create() {
		...
		for (int i = 0; i < 100; i++) {
			cavemen[i] = new Caveman((float) Math.random()
					* Gdx.graphics.getWidth(), (float) Math.random()
					* Gdx.graphics.getHeight(), Math.random() > 0.5 ? true
					: false);
		}
	...
	public void render() {
		...
		for (int i = 0; i < cavemen.length; i++) {
			...
			TextureRegion frame = caveman.headsLeft ? leftWalk.getKeyFrame(
					caveman.stateTime, true) : rightWalk.getKeyFrame(
