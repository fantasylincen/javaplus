...
public class BodyEditorLoader {
	 ...
	 */
	public void attachFixture(IBody body, String name, FixtureDef fd, float scale) {
	...
	private RigidBodyModel readRigidBody(JsonValue bodyElem) {
		...
		rbModel.origin.setX(originElem.get("x").asFloat());
		rbModel.origin.setY(originElem.get("y").asFloat());
		...
		for (int i = 0; i < circlesElem.size; i++) {
			...
			circle.center.setX(circleElem.get("cx").asFloat());
			circle.center.setY(circleElem.get("cy").asFloat());
