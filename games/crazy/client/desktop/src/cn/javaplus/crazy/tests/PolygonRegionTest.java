...
public class PolygonRegionTest extends GdxTest {
	...
	public void create() {
		...
		region = loader.load(new TextureRegion(texture),
				Gdx.files.internal("data/tree.psh"));

		// create a region from an arbitrary set of vertices (a triangle in this
		// case)
		region2 = new PolygonRegion(new TextureRegion(texture), new float[] {
				0, 0, 100, 100, 0, 100 }, new short[] { 0, 1, 2 });
	...
	public void render() {
		...
		debugRenderer.draw(region2, x + width + 10, -128, 0, 0, 256, 256, 1, 1,
				0);
	}
	...
	public class PolygonRegionDebugRenderer implements Disposable {
		...
		public void draw(PolygonRegion region, float x, float y, float originX,
			...
			// construct corner points, start from top left and go counter
			// clockwise
			final float p1x = fx1;
			final float p1y = fy1;
			final float p2x = fx1;
