...
public class BitmapFontTest extends GdxTest {
	...
	public void create() {
		...
		multiPageFont = new BitmapFont(
				Gdx.files.internal("data/multipagefont.fnt"));
	...
	public void render() {
		...
		if (false) {
			alignmentWidth = 0;
			font.drawMultiLine(spriteBatch, text, x, viewHeight - y,
					alignmentWidth, HAlignment.RIGHT);
		}
		...
		if (true) {
			...
			// font.drawMultiLine(spriteBatch, text, x, viewHeight - y,
			// alignmentWidth, HAlignment.RIGHT);
			font.drawWrapped(spriteBatch, text, x, viewHeight - y,
					alignmentWidth, HAlignment.RIGHT);
		}

		// 'R' and 'p' are in different pages
		String txt2 = "this font uses " + multiPageFont.getRegions().length
				+ " texture pages: RpRpRpRpRpNM";
