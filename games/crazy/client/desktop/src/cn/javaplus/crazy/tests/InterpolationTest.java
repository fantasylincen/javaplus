...
public class InterpolationTest extends GdxTest {
	...
	Vector2 startPosition = new Vector2(), targetPosition = new Vector2(),
			position = new Vector2();

	/** resets {@link #startPosition} and {@link #targetPosition} */
	void resetPositions() {
		startPosition.set(stage.getWidth() - stage.getWidth() / 5f,
				stage.getHeight() - stage.getHeight() / 5f);
	...
	Vector2 getPosition(float time) {
		...
		position.scl(getInterpolation(selectedInterpolation).apply(
				time / duration));
	...
	public void create() {
		...
		Field[] interpolationFields = ClassReflection
				.getFields(Interpolation.class);

		// see how many fields are actually interpolations (for safety; other
		// fields may be added with future)
		int interpolationMembers = 0;
		for (int i = 0; i < interpolationFields.length; i++)
			if (Interpolation.class.isAssignableFrom(interpolationFields[i]
					.getDeclaringClass()))
		...
		Gdx.input.setInputProcessor(new InputMultiplexer(new InputAdapter() {
			public boolean scrolled(int amount) {
				...
				duration -= amount / 15f;
				duration = MathUtils
						.clamp(duration, 0, Float.POSITIVE_INFINITY);
				return true;
			}
			...
			public boolean touchDown(int screenX, int screenY, int pointer,
					int button) {
				if (!Float.isNaN(time)) // if "walking" was interrupted by this
										// touch down event
					startPosition.set(getPosition(time)); // set startPosition
															// to the current
															// position
				targetPosition.set(stage
						.screenToStageCoordinates(targetPosition.set(screenX,
								screenY)));
				time = 0;
				return true;
			}
	...
	public void render() {
		...
		float bottomLeftX = Gdx.graphics.getWidth() / 2 - graphSize / 2, bottomLeftY = Gdx.graphics
				.getHeight() / 2 - graphSize / 2;
		...
		list.getStyle().font.draw(stage.getBatch(), text, bottomLeftX
				+ graphSize / 2 - list.getStyle().font.getBounds(text).width
				/ 2,
				bottomLeftY + graphSize + list.getStyle().font.getLineHeight());
		stage.getBatch().end();

		renderer.begin(ShapeType.Line);
		renderer.rect(bottomLeftX, bottomLeftY, graphSize, graphSize); // graph
																		// bounds
		...
		for (float step = 0; step <= steps; step++) {
			...
			float x = bottomLeftX + graphSize * percent, y = bottomLeftY
					+ graphSize * interpolation.apply(percent);
		...
		if (time > duration) {
			...
			startPosition.set(targetPosition); // set startPosition to
												// targetPosition for next click
		}
		// draw time marker
		renderer.line(bottomLeftX + graphSize * time / duration, bottomLeftY,
				bottomLeftX + graphSize * time / duration, bottomLeftY
						+ graphSize);
		// draw path
