...
public class YDownTest extends GdxTest {
	...
	public void create() {
		...
		// a stage which uses our y-down camera and a simple actor (see MyActor
		// below),
	...
	public void render() {
		...
		batch.begin();
		// drawing a region, x and y will be the top left corner of the region,
		// would be bottom left
		// with y-up.
		batch.draw(region, 20, 100);
		// drawing text, x and y will be the top left corner for text, same as
		// with y-up
		...
		// drawing a sprite created from an atlas, FIXME wut?!
		// AtlasSprite#setPosition seems to be wrong
	...
	/**
	 * A very simple actor implementation that does not obey
	 * rotation/scale/origin set on the actor. Allows dragging of the actor.
	 * 
	 * @author mzechner
	 */
		...
		public MyActor(TextureRegion region) {
			...
			addListener(new InputListener() {
				public boolean touchDown(InputEvent event, float x, float y,
					...
					// are given relative to the actor's upper left corner (0,
					// 0)
					lastX = x;
					lastY = y;
					return true;
				...
				public void touchDragged(InputEvent event, float x, float y,
					...
					// adjust the actor's position by (current mouse position -
					// last mouse position)
					// in the actor's coordinate system.
					moveBy(x - lastX, y - lastY);

					// save the current mouse position as the basis for the next
					// drag event.
					// we adjust by the same delta so next time drag is called,
					// lastX/lastY
