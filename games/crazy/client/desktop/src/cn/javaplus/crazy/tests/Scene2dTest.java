...
public class Scene2dTest extends GdxTest {
	...
	public void create() {
		...
		final TextureRegion region = new TextureRegion(new Texture(
				"data/badlogic.jpg"));
		...
		HorizontalGroup h = new HorizontalGroup().space(5).reverse().pad(5)
				.fill();
		for (int i = 0; i < 5; i++)
		...
		button.addListener(new ActorGestureListener() {
			...
			public void pan(InputEvent event, float x, float y, float deltaX,
				...
				if (deltaX < 0)
					System.out.println("panning " + deltaX + ", " + deltaY
							+ " " + event.getTarget());
			}
		});
		...
		// actor.addAction(parallel(moveTo(250, 250, 2, elasticOut), color(RED,
		// 6), delay(0.5f), rotateTo(180, 5, swing)));
		// actor.addAction(forever(sequence(scaleTo(2, 2, 0.5f), scaleTo(1, 1,
		// 0.5f), delay(0.5f))));
