...
public class DefaultJSONParser extends AbstractJSONParser implements Closeable {
	...
	public void parseArray(Type type, Collection array, Object fieldName) {
		...
		try {
			for (int i = 0;; ++i) {
				...
				if (int.class == type) {
					Object val = IntegerDeserializer.instance.deserialze(this,
							null, null);
					array.add(val);
				} else if (String.class == type) {
					String value;
		...
		for (;;) {
			...
			if (fieldDeser == null) {
				...
				if (fieldClass == int.class) {
					...
					args[0] = IntegerDeserializer.instance.deserialze(this,
							fieldType, null);
					...
					lexer.nextTokenWithColon(JSONToken.LITERAL_INT);
					args[0] = LongDeserializer.instance.deserialze(this,
							fieldType, null);
				} else {
					...
					lexer.nextTokenWithColon(fieldValueDeserializer
							.getFastMatchToken());
					args[0] = fieldValueDeserializer.deserialze(this,
							fieldType, null);
				}
	...
	public void checkListResolve(Collection array) {
		if (resolveStatus == NeedToResolve) {
			...
			task.setFieldDeserializer(new ListResolveFieldDeserializer(this,
					list, index));
	...
	public final void parseArray(final Collection array, Object fieldName) {
		...
		if (lexer.token() != JSONToken.LBRACKET) {
			throw new JSONException("syntax error, expect [, actual "
					+ JSONToken.name(lexer.token()) + ", pos " + lexer.pos());
		}
