...
public final class NativeLibraryLoader {
	...
	private static final InternalLogger logger = InternalLoggerFactory
			.getInstance(NativeLibraryLoader.class);
	...
	static {
		OSNAME = SystemPropertyUtil.get("os.name", "").toLowerCase(Locale.US)
				.replaceAll("[^a-z0-9]+", "");
	...
	private static File tmpdir() {
		...
		logger.warn("Failed to get the temporary directory; falling back to: "
				+ f);
		return f;
	}
	...
	public static void load(String name, ClassLoader loader) {
		...
		if (url == null && isOSX()) {
			if (path.endsWith(".jnilib")) {
				url = loader.getResource(NATIVE_RESOURCE_HOME + "lib" + name
						+ ".dynlib");
			} else {
				url = loader.getResource(NATIVE_RESOURCE_HOME + "lib" + name
						+ ".jnilib");
