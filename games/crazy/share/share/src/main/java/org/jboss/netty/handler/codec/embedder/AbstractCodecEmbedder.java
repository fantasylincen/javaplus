...
abstract class AbstractCodecEmbedder<E> implements CodecEmbedder<E> {
	 ...
	 * 
	 * @param bufferFactory
	 *            the {@link ChannelBufferFactory} to be used when creating a
	 *            new buffer.
	 */
	protected AbstractCodecEmbedder(ChannelBufferFactory bufferFactory,
	...
	private final class EmbeddedChannelSink implements ChannelSink,
		...
		private void handleEvent(ChannelEvent e) {
			if (e instanceof MessageEvent) {
				boolean offered = productQueue.offer(((MessageEvent) e)
						.getMessage());
				assert offered;
			} else if (e instanceof ExceptionEvent) {
				throw new CodecEmbedderException(
	...
	private static final class EmbeddedChannelPipeline extends
		...
		protected void notifyHandlerException(ChannelEvent e, Throwable t) {
			while (t instanceof ChannelPipelineException
					&& t.getCause() != null) {
				t = t.getCause();
			}
