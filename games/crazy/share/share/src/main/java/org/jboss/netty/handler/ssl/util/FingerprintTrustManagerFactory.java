 ...
 * {@code openssl} command. For example:
 * 
 * <pre>
 ...
 * </p>
 */
public final class FingerprintTrustManagerFactory extends
		SimpleTrustManagerFactory {

	private static final Pattern FINGERPRINT_PATTERN = Pattern
			.compile("^[0-9a-fA-F:]+$");
	private static final Pattern FINGERPRINT_STRIP_PATTERN = Pattern
			.compile(":");
		...
		private void checkTrusted(String type, X509Certificate[] chain)
			...
			if (!found) {
				throw new CertificateException(type
						+ " certificate with unknown fingerprint: "
						+ cert.getSubjectDN());
			}
		}
		...
		for (byte[] f : fingerprints) {
			...
			if (f.length != SHA1_BYTE_LEN) {
				throw new IllegalArgumentException("malformed fingerprint: "
						+ ChannelBuffers.hexDump(ChannelBuffers
								.wrappedBuffer(f)) + " (expected: SHA1)");
			}
			list.add(f.clone());
		}
	...
	private static byte[][] toFingerprintArray(Iterable<String> fingerprints) {
		...
		for (String f : fingerprints) {
			...
			for (int i = 0; i < farr.length; i++) {
				int strIdx = i << 1;
				farr[i] = (byte) Integer.parseInt(
						f.substring(strIdx, strIdx + 2), 16);
			}
		}
