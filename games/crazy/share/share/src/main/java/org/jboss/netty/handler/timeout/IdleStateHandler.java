...
public class IdleStateHandler extends SimpleChannelUpstreamHandler implements
	...
	public IdleStateHandler(Timer timer, int readerIdleTimeSeconds,
		...
		this(timer, readerIdleTimeSeconds, writerIdleTimeSeconds,
	 ...
	 */
	public IdleStateHandler(Timer timer, long readerIdleTime,
	...
	private void initialize(ChannelHandlerContext ctx) {
		...
		if (readerIdleTimeMillis > 0) {
			state.readerIdleTimeout = timer.newTimeout(
					new ReaderIdleTimeoutTask(ctx), readerIdleTimeMillis,
					TimeUnit.MILLISECONDS);
		}
		if (writerIdleTimeMillis > 0) {
			state.writerIdleTimeout = timer.newTimeout(
					new WriterIdleTimeoutTask(ctx), writerIdleTimeMillis,
					TimeUnit.MILLISECONDS);
		}
		if (allIdleTimeMillis > 0) {
			state.allIdleTimeout = timer.newTimeout(
					new AllIdleTimeoutTask(ctx), allIdleTimeMillis,
					TimeUnit.MILLISECONDS);
		}
	}
	...
	private void fireChannelIdle(final ChannelHandlerContext ctx,
			final IdleState state, final long lastActivityTimeMillis) {
	...
	protected void channelIdle(ChannelHandlerContext ctx, IdleState state,
		...
		ctx.sendUpstream(new DefaultIdleStateEvent(ctx.getChannel(), state,
	...
	private final class ReaderIdleTimeoutTask implements TimerTask {
		...
		public void run(Timeout timeout) throws Exception {
			...
			if (nextDelay <= 0) {
				...
				state.readerIdleTimeout = timer.newTimeout(this,
				...
				// shorter delay.
				state.readerIdleTimeout = timer.newTimeout(this, nextDelay,
						TimeUnit.MILLISECONDS);
			}
		}
	}
	...
	private final class WriterIdleTimeoutTask implements TimerTask {
		...
		public void run(Timeout timeout) throws Exception {
			...
			long nextDelay = writerIdleTimeMillis
					- (currentTime - lastWriteTime);
			if (nextDelay <= 0) {
				// Writer is idle - set a new timeout and notify the callback.
				state.writerIdleTimeout = timer.newTimeout(this,
				...
				// shorter delay.
				state.writerIdleTimeout = timer.newTimeout(this, nextDelay,
						TimeUnit.MILLISECONDS);
			}
		}
	}
