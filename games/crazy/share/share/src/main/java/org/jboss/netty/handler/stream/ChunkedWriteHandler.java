...
public class ChunkedWriteHandler implements ChannelUpstreamHandler,
		ChannelDownstreamHandler, LifeCycleAwareChannelHandler {

	private static final InternalLogger logger = InternalLoggerFactory
			.getInstance(ChunkedWriteHandler.class);
	...
	private void flush(ChannelHandlerContext ctx, boolean fireNow)
		...
		if (acquired
				&& (!channel.isConnected() || channel.isWritable()
						&& !queue.isEmpty() && !suspend || flushNeeded)) {
			flush(ctx, fireNow);
		}
	}
