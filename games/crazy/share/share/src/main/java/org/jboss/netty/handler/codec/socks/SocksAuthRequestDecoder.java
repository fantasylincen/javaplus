...
/**
 * Decodes {@link ChannelBuffer}s into {@link SocksAuthRequest}. Before
 * returning SocksRequest decoder removes itself from pipeline.
 */
public class SocksAuthRequestDecoder extends
	...
	protected Object decode(ChannelHandlerContext ctx, Channel channel,
		...
		case CHECK_PROTOCOL_VERSION: {
			version = SocksMessage.SubnegotiationVersion.fromByte(buffer
					.readByte());
		...
		case READ_USERNAME: {
			...
			username = buffer.readBytes(fieldLength).toString(
					CharsetUtil.US_ASCII);
			checkpoint(State.READ_PASSWORD);
		}
		case READ_PASSWORD: {
			fieldLength = buffer.readByte();
			password = buffer.readBytes(fieldLength).toString(
					CharsetUtil.US_ASCII);
