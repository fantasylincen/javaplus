...
public class BigEndianHeapChannelBuffer extends HeapChannelBuffer {
	...
	private BigEndianHeapChannelBuffer(byte[] array, int readerIndex,
			int writerIndex) {
	...
	public int getUnsignedMedium(int index) {
		return (array[index] & 0xff) << 16 | (array[index + 1] & 0xff) << 8
				| array[index + 2] & 0xff;
	}
	...
	public ChannelBuffer duplicate() {
		return new BigEndianHeapChannelBuffer(array, readerIndex(),
				writerIndex());
	}
	...
	public ChannelBuffer copy(int index, int length) {
		if (index < 0 || length < 0 || index + length > array.length) {
			throw new IndexOutOfBoundsException(
					"Too many bytes to copy - Need " + (index + length)
							+ ", maximum is " + array.length);
		}
