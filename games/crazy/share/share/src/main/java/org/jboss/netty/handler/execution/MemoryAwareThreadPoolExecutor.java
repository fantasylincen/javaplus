 ...
 * too many tasks in the queue. Both per-{@link Channel} and per-
 * {@link Executor} limitation can be applied.
 * <p>
 ...
 * </ul>
 * Here is an example that demonstrates how to implement an
 ...
 * {@link ChannelEvent}s for the same {@link Channel}. For example, you can even
...
public class MemoryAwareThreadPoolExecutor extends ThreadPoolExecutor {
	...
	private static final InternalLogger logger = InternalLoggerFactory
			.getInstance(MemoryAwareThreadPoolExecutor.class);
	...
	public MemoryAwareThreadPoolExecutor(int corePoolSize,
		...
		this(corePoolSize, maxChannelMemorySize, maxTotalMemorySize,
	...
	public MemoryAwareThreadPoolExecutor(int corePoolSize,
		...
		this(corePoolSize, maxChannelMemorySize, maxTotalMemorySize,
				keepAliveTime, unit, new DefaultObjectSizeEstimator(),
				threadFactory);
	}
	...
	public MemoryAwareThreadPoolExecutor(int corePoolSize,
		...
		if (maxChannelMemorySize < 0) {
			throw new IllegalArgumentException("maxChannelMemorySize: "
					+ maxChannelMemorySize);
		}
		if (maxTotalMemorySize < 0) {
			throw new IllegalArgumentException("maxTotalMemorySize: "
					+ maxTotalMemorySize);
		}
		...
		try {
			Method m = getClass().getMethod("allowCoreThreadTimeOut",
					new Class[] { boolean.class });
			m.invoke(this, Boolean.TRUE);
		} catch (Throwable t) {
			// Java 5
			logger.debug("ThreadPoolExecutor.allowCoreThreadTimeOut() is not "
	 ...
	 * {@link ChannelFuture}'s of the not executed {@link ChannelEventRunnable}
	 * 's.
	 * 
	 * <p>
	...
	public void setObjectSizeEstimator(ObjectSizeEstimator objectSizeEstimator) {
		...
		settings = new Settings(objectSizeEstimator,
	...
	public void setMaxChannelMemorySize(long maxChannelMemorySize) {
		...
		settings = new Settings(settings.objectSizeEstimator,
				maxChannelMemorySize);
	}
	 ...
	 * will get marked as failed via {@link ChannelFuture#setFailure(Throwable)}
	 * .
	 * 
	 * <p>
	...
	public void execute(Runnable command) {
		if (command instanceof ChannelDownstreamEventRunnable) {
			throw new RejectedExecutionException(
					"command must be enclosed with an upstream event.");
		}
	...
	protected void increaseCounter(Runnable task) {
		...
		if (task instanceof ChannelEventRunnable) {
			...
			long channelCounter = getChannelCounter(channel).addAndGet(
					increment);
			// System.out.println("IC: " + channelCounter + ", " + increment);
			if (maxChannelMemorySize != 0
					&& channelCounter >= maxChannelMemorySize
					&& channel.isOpen()) {
	...
	private AtomicLong getChannelCounter(Channel channel) {
		...
		if (counter == null) {
			...
			AtomicLong oldCounter = channelCounters.putIfAbsent(channel,
					counter);
			if (oldCounter != null) {
				counter = oldCounter;
			}
