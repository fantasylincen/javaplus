...
/**
 * A {@link ChannelPipeline} that might perform better at the cost of disabled
 * dynamic insertion and removal of {@link ChannelHandler}s. An attempt to
 * insert, remove, or replace a handler in this pipeline will trigger an
...
public class StaticChannelPipeline implements ChannelPipeline {
	...
	static final InternalLogger logger = InternalLoggerFactory
			.getInstance(StaticChannelPipeline.class);

	private volatile Channel channel;
	private volatile ChannelSink sink;
	private final StaticChannelHandlerContext[] contexts;
	private final int lastIndex;
	private final Map<String, StaticChannelHandlerContext> name2ctx = new HashMap<String, StaticChannelHandlerContext>(
			4);

	/**
	 * Creates a new pipeline from the specified handlers. The names of the
	 * specified handlers are generated automatically; the first handler's name
	 * is {@code "0"}, the second handler's name is {@code "1"}, the third
	 * handler's name is {@code "2"}, and so on.
	 */
	public StaticChannelPipeline(ChannelHandler... handlers) {
		...
		if (nContexts == contexts.length) {
			...
			this.contexts = contexts = new StaticChannelHandlerContext[nContexts];
		...
		for (int i = 0; i < nContexts; i++) {
			...
			StaticChannelHandlerContext ctx = new StaticChannelHandlerContext(
					i, name, h);
			contexts[i] = ctx;
	...
	private final class StaticChannelHandlerContext implements
		...
		StaticChannelHandlerContext(int index, String name,
