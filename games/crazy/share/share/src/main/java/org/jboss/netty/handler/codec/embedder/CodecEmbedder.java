...
public interface CodecEmbedder<E> {
	 ...
	 * 
	 * @return an encoded or decoded object. {@code null} if and only if there
	 *         is no output object left in the product queue.
	 */
	E poll();
	...
	/**
	 * Consumes all encoded or decoded output from the product queue. The output
	 * object is generated by the offered input objects. The behavior of this
	 * method is identical with {@link Collection#toArray()} except that the
	 * product queue is cleared.
	 * 
	 * @return an array of all encoded or decoded objects. An empty array is
	 *         returned if and only if there is no output object left in the
	 *         product queue.
	 */
	Object[] pollAll();
	 ...
	 * Consumes all encoded or decoded output from the product queue. The output
	 * object is generated by the offered input objects. The behavior of this
	 * method is identical with {@link Collection#toArray(Object[])} except that
