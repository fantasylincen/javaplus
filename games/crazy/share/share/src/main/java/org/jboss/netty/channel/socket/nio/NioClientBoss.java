...
public final class NioClientBoss extends AbstractNioSelector implements Boss {
	...
	NioClientBoss(Executor bossExecutor, Timer timer,
	...
	@Override
	protected ThreadRenamingRunnable newThreadRenamingRunnable(int id,
			ThreadNameDeterminer determiner) {
		return new ThreadRenamingRunnable(this, "New I/O boss #" + id,
				determiner);
	}
	...
	private void processSelectedKeys(Set<SelectionKey> selectedKeys) {
		...
		for (Iterator<SelectionKey> i = selectedKeys.iterator(); i.hasNext();) {
			...
			try {
				...
				NioClientSocketChannel ch = (NioClientSocketChannel) k
						.attachment();
	...
	private static void processConnectTimeout(Set<SelectionKey> keys,
	...
	private static void connect(SelectionKey k) throws IOException {
		...
		try {
			...
			ConnectException newE = new ConnectException(e.getMessage() + ": "
					+ ch.requestedRemoteAddress);
		...
		public void run() {
			...
			if (timeout > 0) {
				if (!channel.isConnected()) {
					channel.timoutTimer = timer.newTimeout(wakeupTask, timeout,
							TimeUnit.MILLISECONDS);
				}
			}
			try {
				channel.channel.register(boss.selector,
						SelectionKey.OP_CONNECT, channel);
			...
			if (connectTimeout > 0) {
				channel.connectDeadlineNanos = System.nanoTime()
						+ connectTimeout * 1000000L;
			}
		}
	}
}
