...
public abstract class AbstractNioBossPool<E extends Boss> implements
		BossPool<E>, ExternalResourceReleasable {
	...
	private static final InternalLogger logger = InternalLoggerFactory
			.getInstance(AbstractNioBossPool.class);
	...
	AbstractNioBossPool(Executor bossExecutor, int bossCount, boolean autoInit) {
		...
		if (bossCount <= 0) {
			throw new IllegalArgumentException("bossCount (" + bossCount + ") "
	...
	private void waitForBossThreads() {
		...
		for (Boss boss : bosses) {
			...
			try {
				...
				} else if (!selector.startupLatch.await(waitTime,
		...
		if (warn) {
			logger.warn("Failed to get all boss threads ready within "
