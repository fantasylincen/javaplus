...
public class CompositeChannelBuffer extends AbstractChannelBuffer {
	...
	public CompositeChannelBuffer(ByteOrder endianness,
			List<ChannelBuffer> buffers, boolean gathering) {
		order = endianness;
	...
	public List<ChannelBuffer> decompose(int index, int length) {
		...
		if (index + length > capacity()) {
			throw new IndexOutOfBoundsException(
					"Too many bytes to decompose - Need " + (index + length)
							+ ", capacity is " + capacity());
		}

		int componentId = componentId(index);
		List<ChannelBuffer> slice = new ArrayList<ChannelBuffer>(
				components.length);
	...
	public short getShort(int index) {
		...
		if (index + 2 <= indices[componentId + 1]) {
			return components[componentId].getShort(index
					- indices[componentId]);
	...
	public int getUnsignedMedium(int index) {
		...
		if (index + 3 <= indices[componentId + 1]) {
			return components[componentId].getUnsignedMedium(index
					- indices[componentId]);
		} else if (order() == ByteOrder.BIG_ENDIAN) {
	...
	public long getLong(int index) {
		...
		if (index + 8 <= indices[componentId + 1]) {
			return components[componentId]
					.getLong(index - indices[componentId]);
		} else if (order() == ByteOrder.BIG_ENDIAN) {
	...
	public void getBytes(int index, byte[] dst, int dstIndex, int length) {
		if (index > capacity() - length || dstIndex > dst.length - length) {
			throw new IndexOutOfBoundsException(
					"Too many bytes to read - Needs " + (index + length)
							+ ", maximum is " + capacity() + " or "
							+ dst.length);
		}
		if (index < 0) {
	...
	public void getBytes(int index, ByteBuffer dst) {
		...
		if (index > capacity() - length) {
			throw new IndexOutOfBoundsException(
					"Too many bytes to be read - Needs " + (index + length)
							+ ", maximum is " + capacity());
		}
		if (index < 0) {
			throw new IndexOutOfBoundsException("Index must be >= 0");
	...
	public void setShort(int index, int value) {
		int componentId = componentId(index);
		if (index + 2 <= indices[componentId + 1]) {
			components[componentId].setShort(index - indices[componentId],
					value);
		} else if (order() == ByteOrder.BIG_ENDIAN) {
	...
	public void setMedium(int index, int value) {
		int componentId = componentId(index);
		if (index + 3 <= indices[componentId + 1]) {
			components[componentId].setMedium(index - indices[componentId],
					value);
		} else if (order() == ByteOrder.BIG_ENDIAN) {
	...
	public void setLong(int index, long value) {
		...
		if (index + 8 <= indices[componentId + 1]) {
			components[componentId]
					.setLong(index - indices[componentId], value);
		} else if (order() == ByteOrder.BIG_ENDIAN) {
	...
	public void setBytes(int index, ByteBuffer src) {
		...
		if (index > capacity() - length) {
			throw new IndexOutOfBoundsException(
					"Too many bytes to be written - Needs " + (index + length)
							+ ", maximum is " + capacity());
		}
	...
	public int setBytes(int index, InputStream in, int length)
		...
		if (index > capacity() - length) {
			throw new IndexOutOfBoundsException(
					"Too many bytes to write - Needs " + (index + length)
							+ ", maximum is " + capacity());
		}
	...
	public ChannelBuffer copy(int index, int length) {
		...
		if (index > capacity() - length) {
			throw new IndexOutOfBoundsException(
					"Too many bytes to copy - Needs " + (index + length)
							+ ", maximum is " + capacity());
		}
	...
	private void copyTo(int index, int length, int componentId,
			ChannelBuffer dst) {
		int dstIndex = 0;
		int i = componentId;
	...
	public ByteBuffer[] toByteBuffers(int index, int length) {
		if (index + length > capacity()) {
			throw new IndexOutOfBoundsException(
					"Too many bytes to convert - Needs" + (index + length)
							+ ", maximum is " + capacity());
		}
		if (index < 0) {
			throw new IndexOutOfBoundsException("Index must be >= 0");
	...
	public void discardReadBytes() {
		...
		final ChannelBuffer padding = ChannelBuffers.buffer(order(),
				localReaderIndex);
