 ...
 * <p>
 * The encoding and decoding algorithm in this class has been derived from <a
 * href="http://iharder.sourceforge.net/current/java/base64/">Robert Harder's
...
public final class Base64 {
	...
	public static ChannelBuffer encode(ChannelBuffer src,
			Base64Dialect dialect, ChannelBufferFactory bufferFactory) {
	...
	public static ChannelBuffer encode(ChannelBuffer src, boolean breakLines,
		...
		return encode(src, breakLines, dialect,
	...
	public static ChannelBuffer encode(ChannelBuffer src, boolean breakLines,
		...
		ChannelBuffer dest = encode(src, src.readerIndex(),
				src.readableBytes(), breakLines, dialect, bufferFactory);
	...
	public static ChannelBuffer encode(ChannelBuffer src, int off, int len,
		...
		int len43 = len * 4 / 3;
		ChannelBuffer dest = bufferFactory.getBuffer(src.order(), len43
	...
	private static void encode3to4(ChannelBuffer src, int srcOffset,
			int numSigBytes, ChannelBuffer dest, int destOffset,
			Base64Dialect dialect) {
		...
		// int.
		int inBuff = (numSigBytes > 0 ? src.getByte(srcOffset) << 24 >>> 8 : 0)
				| (numSigBytes > 1 ? src.getByte(srcOffset + 1) << 24 >>> 16
						: 0)
				| (numSigBytes > 2 ? src.getByte(srcOffset + 2) << 24 >>> 24
						: 0);
	...
	public static ChannelBuffer decode(ChannelBuffer src,
		...
		ChannelBuffer dest = decode(src, src.readerIndex(),
				src.readableBytes(), dialect, bufferFactory);
		src.readerIndex(src.writerIndex());
		return dest;
	}
	...
	public static ChannelBuffer decode(ChannelBuffer src, int off, int len,
			Base64Dialect dialect) {
		return decode(src, off, len, dialect,
				HeapChannelBufferFactory.getInstance());
	}
			...
			if (sbiDecode >= WHITE_SPACE_ENC) { // White space, Equals sign or
				...
				if (sbiDecode >= EQUALS_SIGN_ENC) { // Equals sign or better?
					...
					if (b4Posn > 3) { // Quartet built?
						outBuffPosn += decode4to3(b4, 0, dest, outBuffPosn,
								dialect);
						b4Posn = 0;
	...
	private static int decode4to3(byte[] src, int srcOffset,
		...
		if (src[srcOffset + 2] == EQUALS_SIGN) {
			// Example: Dk==
			int outBuff = (DECODABET[src[srcOffset]] & 0xFF) << 18
			...
			int outBuff;
			try {
				outBuff = (DECODABET[src[srcOffset]] & 0xFF) << 18
						| (DECODABET[src[srcOffset + 1]] & 0xFF) << 12
						| (DECODABET[src[srcOffset + 2]] & 0xFF) << 6
