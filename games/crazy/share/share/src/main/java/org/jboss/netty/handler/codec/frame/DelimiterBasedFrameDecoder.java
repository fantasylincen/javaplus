 ...
 * delimiter which produces the shortest frame. For example, if you have the
 ...
 * </pre>
 * 
 * a {@link DelimiterBasedFrameDecoder}{@code (}
 * {@link Delimiters#lineDelimiter() Delimiters.lineDelimiter()}{@code )} will
...
public class DelimiterBasedFrameDecoder extends FrameDecoder {
	...
	public DelimiterBasedFrameDecoder(int maxFrameLength,
		...
		this(maxFrameLength, stripDelimiter, failFast,
				new ChannelBuffer[] { delimiter.slice(delimiter.readerIndex(),
	...
	public DelimiterBasedFrameDecoder(int maxFrameLength,
		...
		if (isLineBased(delimiters) && !isSubclass()) {
			lineBasedDecoder = new LineBasedFrameDecoder(maxFrameLength,
					stripDelimiter, failFast);
			...
			for (int i = 0; i < delimiters.length; i++) {
				...
				validateDelimiter(d);
				this.delimiters[i] = d
						.slice(d.readerIndex(), d.readableBytes());
			}
	...
	private static boolean isLineBased(final ChannelBuffer[] delimiters) {
		...
		return a.capacity() == 2 && b.capacity() == 1 && a.getByte(0) == '\r'
				&& a.getByte(1) == '\n' && b.getByte(0) == '\n';
	}
