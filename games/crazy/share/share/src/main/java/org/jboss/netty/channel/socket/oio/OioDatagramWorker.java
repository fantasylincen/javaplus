...
class OioDatagramWorker extends AbstractOioWorker<OioDatagramChannel> {
	...
	boolean process() throws IOException {
		...
		ReceiveBufferSizePredictor predictor = channel.getConfig()
		...
		fireMessageReceived(channel, channel.getConfig().getBufferFactory()
				.getBuffer(buf, 0, packet.getLength()),
				packet.getSocketAddress());
		return true;
	}

	static void write(OioDatagramChannel channel, ChannelFuture future,
		...
		try {
			...
			if (nioBuf.hasArray()) {
				...
				packet = new DatagramPacket(nioBuf.array(),
						nioBuf.arrayOffset() + offset, length);
			} else {
