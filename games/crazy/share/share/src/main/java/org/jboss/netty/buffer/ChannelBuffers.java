...
public final class ChannelBuffers {
	...
	public static ChannelBuffer directBuffer(ByteOrder endianness, int capacity) {
		...
		ChannelBuffer buffer = new ByteBufferBackedChannelBuffer(ByteBuffer
				.allocateDirect(capacity).order(endianness));
		buffer.clear();
		return buffer;
	}
	...
	public static ChannelBuffer dynamicBuffer(int estimatedLength,
		...
		return new DynamicChannelBuffer(factory.getDefaultOrder(),
	 ...
	 */
	public static ChannelBuffer wrappedBuffer(byte[] array, int offset,
			int length) {
		...
		if (offset == 0) {
			if (length == array.length) {
				...
				if (length == 0) {
					...
					return new TruncatedChannelBuffer(wrappedBuffer(endianness,
							array), length);
				}
			}
		} else {
			if (length == 0) {
				...
				return new SlicedChannelBuffer(
						wrappedBuffer(endianness, array), offset, length);
			}
		}
	}
	...
	public static ChannelBuffer wrappedBuffer(ByteBuffer buffer) {
		...
		if (buffer.hasArray()) {
			return wrappedBuffer(buffer.order(), buffer.array(),
	...
	public static ChannelBuffer wrappedBuffer(ByteOrder endianness,
		...
		default:
			...
			final List<ChannelBuffer> components = new ArrayList<ChannelBuffer>(
					arrays.length);
			for (byte[] a : arrays) {
				if (a == null) {
					break;
	...
	private static ChannelBuffer compositeBuffer(ByteOrder endianness,
			List<ChannelBuffer> components, boolean gathering) {
	 ...
	 *             other
	 */
	public static ChannelBuffer wrappedBuffer(boolean gathering,
			...
			for (ChannelBuffer c : buffers) {
				...
				if (c.readable()) {
					...
					if (c instanceof CompositeChannelBuffer) {
						...
						components.addAll(((CompositeChannelBuffer) c)
								.decompose(c.readerIndex(), c.readableBytes()));
					} else {
	 ...
	 * respectively.
	 */
	public static ChannelBuffer copiedBuffer(byte[] array, int offset,
			int length) {
	 ...
	 */
	public static ChannelBuffer copiedBuffer(CharSequence string,
			Charset charset) {
	 ...
	 */
	public static ChannelBuffer copiedBuffer(CharSequence string, int offset,
	 ...
	 */
	public static ChannelBuffer copiedBuffer(ByteOrder endianness,
			CharSequence string, Charset charset) {
		if (string == null) {
	...
	public static ChannelBuffer copiedBuffer(ByteOrder endianness,
		...
		if (string instanceof CharBuffer) {
			...
			if (buf.hasArray()) {
				return copiedBuffer(endianness, buf.array(), buf.arrayOffset()
						+ buf.position() + offset, length, charset);
			}
		...
		return copiedBuffer(endianness,
				CharBuffer.wrap(string, offset, offset + length), charset);
	}
	 ...
	 */
	public static ChannelBuffer copiedBuffer(char[] array, int offset,
			int length, Charset charset) {
	...
	public static ChannelBuffer copiedBuffer(ByteOrder endianness,
		...
		return copiedBuffer(endianness, CharBuffer.wrap(array, offset, length),
				charset);
	}

	private static ChannelBuffer copiedBuffer(ByteOrder endianness,
			CharBuffer buffer, Charset charset) {
	...
	public static ChannelBuffer hexDump(String hexString) {
		...
		for (int i = 0; i < len; i += 2) {
			hexData[i / 2] = (byte) ((Character.digit(hexString.charAt(i), 16) << 4) + Character
	...
	public static String hexDump(ChannelBuffer buffer, int fromIndex, int length) {
		...
		for (; srcIdx < endIndex; srcIdx++, dstIdx += 2) {
			System.arraycopy(HEXDUMP_TABLE,
					buffer.getUnsignedByte(srcIdx) << 1, buf, dstIdx, 2);
		}
	 ...
	 * implementing a new buffer type.
	 */
	public static int indexOf(ChannelBuffer buffer, int fromIndex, int toIndex,
			byte value) {
	...
	private static int firstIndexOf(ChannelBuffer buffer, int fromIndex,
			int toIndex, byte value) {
	...
	private static int lastIndexOf(ChannelBuffer buffer, int fromIndex,
			int toIndex, byte value) {
	...
	private static int firstIndexOf(ChannelBuffer buffer, int fromIndex,
			int toIndex, ChannelBufferIndexFinder indexFinder) {
		fromIndex = Math.max(fromIndex, 0);
		if (fromIndex >= toIndex || buffer.capacity() == 0) {
			return -1;
		}
	...
	static ByteBuffer encodeString(CharBuffer src, Charset charset) {
		...
		final ByteBuffer dst = ByteBuffer.allocate((int) ((double) src
				.remaining() * encoder.maxBytesPerChar()));
		try {
