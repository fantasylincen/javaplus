...
public class HashedWheelTimer implements Timer {
	...
	static final InternalLogger logger = InternalLoggerFactory
			.getInstance(HashedWheelTimer.class);
	 ...
	 */
	public HashedWheelTimer(ThreadFactory threadFactory,
			ThreadNameDeterminer determiner, long tickDuration, TimeUnit unit,
			int ticksPerWheel) {
		...
		private void transferTimeoutsToBuckets() {
			...
			for (int i = 0; i < 100000; i++) {
				...
				if (timeout.state() == HashedWheelTimeout.ST_CANCELLED
						|| !timeout.compareAndSetState(
								HashedWheelTimeout.ST_INIT,
								HashedWheelTimeout.ST_IN_BUCKET)) {
	...
	private static final class HashedWheelTimeout implements Timeout {
		...
		public void cancel() {
			...
			if (state != ST_IN_BUCKET
					&& compareAndSetState(ST_INIT, ST_CANCELLED)) {
