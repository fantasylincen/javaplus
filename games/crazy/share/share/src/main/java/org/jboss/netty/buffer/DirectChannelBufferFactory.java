...
public class DirectChannelBufferFactory extends AbstractChannelBufferFactory {
	...
	private static final DirectChannelBufferFactory INSTANCE_BE = new DirectChannelBufferFactory(
	...
	public DirectChannelBufferFactory(ByteOrder defaultOrder,
		...
		if (preallocatedBufferCapacity <= 0) {
			throw new IllegalArgumentException(
					"preallocatedBufCapacity must be greater than 0: "
	...
	public ChannelBuffer getBuffer(ByteOrder order, byte[] array, int offset,
			int length) {
		if (array == null) {
	...
	private ChannelBuffer allocateBigEndianBuffer(int capacity) {
		...
		synchronized (bigEndianLock) {
			if (preallocatedBEBuf == null) {
				preallocatedBEBuf = ChannelBuffers.directBuffer(
						ByteOrder.BIG_ENDIAN, preallocatedBufCapacity);
	...
	private ChannelBuffer allocateLittleEndianBuffer(int capacity) {
		...
		synchronized (littleEndianLock) {
			if (preallocatedLEBuf == null) {
				preallocatedLEBuf = ChannelBuffers.directBuffer(
						ByteOrder.LITTLE_ENDIAN, preallocatedBufCapacity);
