...
public final class NioServerBoss extends AbstractNioSelector implements Boss {
	...
	protected void process(Selector selector) {
		...
		for (Iterator<SelectionKey> i = selectedKeys.iterator(); i.hasNext();) {
			...
			i.remove();
			NioServerSocketChannel channel = (NioServerSocketChannel) k
					.attachment();
	...
	private static void registerAcceptedChannel(NioServerSocketChannel parent,
		...
		try {
			...
			ChannelPipeline pipeline = parent.getConfig().getPipelineFactory()
					.getPipeline();
			NioWorker worker = parent.workerPool.nextWorker();
			worker.register(new NioAcceptedSocketChannel(parent.getFactory(),
					pipeline, parent, sink, acceptedSocket, worker,
					currentThread), null);
		} catch (Exception e) {
			if (logger.isWarnEnabled()) {
			...
			try {
				...
				if (logger.isWarnEnabled()) {
					logger.warn("Failed to close a partially accepted socket.",
							e2);
				}
			}
		}
	...
	protected ThreadRenamingRunnable newThreadRenamingRunnable(int id,
		...
		return new ThreadRenamingRunnable(this, "New I/O server boss #" + id,
				determiner);
	}
	...
	private final class RegisterTask implements Runnable {
		...
		public RegisterTask(final NioServerSocketChannel channel,
				final ChannelFuture future, final SocketAddress localAddress) {
