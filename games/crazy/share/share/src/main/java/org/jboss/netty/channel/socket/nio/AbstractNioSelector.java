...
abstract class AbstractNioSelector implements NioSelector {
	...
	public void run() {
		...
		for (;;) {
			...
			try {
				...
				if (SelectorUtil.EPOLL_BUG_WORKAROUND && selected == 0
						&& !wakenupFromLoop && !wakenUp.get()) {
	...
	private void openSelector(ThreadNameDeterminer determiner) {
		...
		try {
			DeadLockProofWorker.start(executor,
					newThreadRenamingRunnable(id, determiner));
			success = true;
		} finally {
			if (!success) {
	...
	public void shutdown() {
		...
		try {
			...
			logger.error("Interrupted while wait for resources to be released #"
					+ id);
	...
	protected abstract ThreadRenamingRunnable newThreadRenamingRunnable(int id,
			ThreadNameDeterminer determiner);

	protected abstract Runnable createRegisterTask(Channel channel,
