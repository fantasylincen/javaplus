...
/**
 * An abstract class that defines a SocksMessage, providing common properties
 * for {@link SocksRequest} and {@link SocksResponse}.
 * 
 * @see SocksRequest
 * @see SocksResponse
 */
...
public abstract class SocksMessage {
	...
	public enum AuthScheme {
		NO_AUTH((byte) 0x00), AUTH_GSSAPI((byte) 0x01), AUTH_PASSWORD(
				(byte) 0x02), UNKNOWN((byte) 0xff);
	...
	public enum CmdType {
		CONNECT((byte) 0x01), BIND((byte) 0x02), UDP((byte) 0x03), UNKNOWN(
				(byte) 0xff);
	...
	public enum AddressType {
		IPv4((byte) 0x01), DOMAIN((byte) 0x03), IPv6((byte) 0x04), UNKNOWN(
				(byte) 0xff);
	...
	public enum CmdStatus {
		SUCCESS((byte) 0x00), FAILURE((byte) 0x01), FORBIDDEN((byte) 0x02), NETWORK_UNREACHABLE(
				(byte) 0x03), HOST_UNREACHABLE((byte) 0x04), REFUSED(
				(byte) 0x05), TTL_EXPIRED((byte) 0x06), COMMAND_NOT_SUPPORTED(
				(byte) 0x07), ADDRESS_NOT_SUPPORTED((byte) 0x08), UNASSIGNED(
				(byte) 0xff);
	...
	/**
	 * Encode socks message into its byte representation and write it into
	 * byteBuf
	 * 
	 * @see ChannelBuffer
	 */
