...
public abstract class AbstractChannelBuffer implements ChannelBuffer {
	...
	public void getBytes(int index, ChannelBuffer dst, int length) {
		if (length > dst.writableBytes()) {
			throw new IndexOutOfBoundsException(
					"Too many bytes to be read: Need " + length
							+ ", maximum is " + dst.writableBytes());
		}
	...
	public void setBytes(int index, ChannelBuffer src, int length) {
		if (length > src.readableBytes()) {
			throw new IndexOutOfBoundsException(
					"Too many bytes to write: Need " + length + ", maximum is "
							+ src.readableBytes());
		}
	...
	public byte readByte() {
		if (readerIndex == writerIndex) {
			throw new IndexOutOfBoundsException(
					"Readable byte limit exceeded: " + readerIndex);
		}
	...
	public void skipBytes(int length) {
		...
		if (newReaderIndex > writerIndex) {
			throw new IndexOutOfBoundsException(
					"Readable bytes exceeded - Need " + newReaderIndex
							+ ", maximum is " + writerIndex);
		}
	...
	public int skipBytes(ChannelBufferIndexFinder firstIndexFinder) {
		...
		int newReaderIndex = indexOf(oldReaderIndex, writerIndex,
				firstIndexFinder);
		if (newReaderIndex < 0) {
			throw new NoSuchElementException();
		}
	...
	public void writeBytes(ChannelBuffer src, int length) {
		if (length > src.readableBytes()) {
			throw new IndexOutOfBoundsException(
					"Too many bytes to write - Need " + length
							+ ", maximum is " + src.readableBytes());
		}
	...
	public String toString(int index, int length, Charset charset) {
		...
		return ChannelBuffers
				.decodeString(toByteBuffer(index, length), charset);
	}
