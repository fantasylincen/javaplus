 ...
 * required bytes. For example, the following {@link FrameDecoder}
 * implementation:
 * 
 * <pre>
 ...
 * complicated unlike the example above. In this case, your decoder might have
 ...
 * times to decode a single message. For example, the following code will not
 * work:
 * 
 * <pre>
...
public abstract class ReplayingDecoder<T extends Enum<T>> extends FrameDecoder {
	...
	@Override
	protected final Object decodeLast(ChannelHandlerContext ctx,
	...
	public void messageReceived(ChannelHandlerContext ctx, MessageEvent e)
		...
		if (cumulation == null) {
			...
			try {
				callDecode(ctx, e.getChannel(), input, replayable,
						e.getRemoteAddress());
			} finally {
				...
				if (readableBytes > 0) {
					...
					boolean copy = readableBytes != inputCapacity
					...
					if (checkpoint > 0) {
						int bytesToPreserve = inputSize
								- (checkpoint - oldReaderIndex);
						if (copy) {
							this.cumulation = cumulation = newCumulationBuffer(
									ctx, bytesToPreserve);
							cumulation.writeBytes(input, checkpoint,
									bytesToPreserve);
						} else {
							this.cumulation = input.slice(checkpoint,
									bytesToPreserve);
						}
						...
						if (copy) {
							this.cumulation = cumulation = newCumulationBuffer(
									ctx, inputSize);
							cumulation.writeBytes(input, oldReaderIndex,
									inputSize);
							cumulation.readerIndex(input.readerIndex());
						} else {
							this.cumulation = cumulation = input.slice(
									oldReaderIndex, inputSize);
							cumulation.readerIndex(input.readerIndex());
						}
					} else {
						if (copy) {
							this.cumulation = cumulation = newCumulationBuffer(
									ctx, input.readableBytes());
	...
	private void callDecode(ChannelHandlerContext context, Channel channel,
			ChannelBuffer input, ChannelBuffer replayableInput,
	...
	protected void cleanup(ChannelHandlerContext ctx, ChannelStateEvent e)
		...
		try {
			...
			Object partiallyDecoded = decodeLast(ctx, e.getChannel(),
					replayable, state);
