...
public class DefaultChannelFuture implements ChannelFuture {
	...
	private static final InternalLogger logger = InternalLoggerFactory
			.getInstance(DefaultChannelFuture.class);
	...
	public static void setUseDeadLockChecker(boolean useDeadLockChecker) {
		if (!useDeadLockChecker && !disabledDeadLockCheckerOnce) {
			...
			if (logger.isDebugEnabled()) {
				logger.debug("The dead lock checker in "
			...
			if (done) {
				...
				if (listener instanceof ChannelFutureProgressListener) {
					if (progressListeners == null) {
						progressListeners = new ArrayList<ChannelFutureProgressListener>(
								1);
					}
					progressListeners
							.add((ChannelFutureProgressListener) listener);
				}
			}
		}
	...
	public boolean setProgress(long amount, long current, long total) {
		...
		synchronized (this) {
			...
			plisteners = progressListeners
					.toArray(new ChannelFutureProgressListener[progressListeners
							.size()]);
		}
	...
	private void notifyProgressListener(ChannelFutureProgressListener l,
		...
		try {
			...
			if (logger.isWarnEnabled()) {
				logger.warn("An exception was thrown by "
						+ ChannelFutureProgressListener.class.getSimpleName()
						+ '.', t);
			}
		}
	}
