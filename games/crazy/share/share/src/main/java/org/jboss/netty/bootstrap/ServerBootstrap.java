 ...
 * deal with some special needs such as degrading the process <a
 * href="http://en.wikipedia.org/wiki/User_identifier_(Unix)">UID</a> from a <a
 * href="http://en.wikipedia.org/wiki/Superuser">superuser</a> to a normal user
 * and changing the current VM security manager for better security. To support
 * such a case, the {@link #setParentHandler(ChannelHandler) parentHandler}
...
public class ServerBootstrap extends Bootstrap {
	...
	private final class Binder extends SimpleChannelUpstreamHandler {
		...
		private final DefaultChannelFuture bindFuture = new DefaultChannelFuture(
				null, false);
		...
		public void channelOpen(ChannelHandlerContext ctx, ChannelStateEvent evt) {
			...
			try {
				evt.getChannel().getConfig()
						.setPipelineFactory(getPipelineFactory());
