 ...
 */
public class ZlibEncoder extends OneToOneStrictEncoder implements
			...
			try {
				...
				if (z.next_out_index != 0) {
					result = ctx
							...
							.getConfig()
							.getBufferFactory()
							.getBuffer(uncompressed.order(), out, 0,
									z.next_out_index);
				} else {
					result = ChannelBuffers.EMPTY_BUFFER;
				}
			} finally {
				// Deference the external references explicitly to tell the VM
				// that
				// the allocated byte arrays are temporary so that the call
				// stack
				// can be utilized.
		...
		synchronized (z) {
			try {
				...
				if (resultCode != JZlib.Z_OK
					...
					future = Channels.failedFuture(ctx.getChannel(), ZlibUtil
							.exception(z, "compression failure", resultCode));
					footer = null;
				} else if (z.next_out_index != 0) {
					future = Channels.future(ctx.getChannel());
					footer = ctx.getChannel().getConfig().getBufferFactory()
							.getBuffer(out, 0, z.next_out_index);
				} else {
