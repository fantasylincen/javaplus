 ...
 * <h4>Boss threads</h4>
 * <p>
 * Each bound {@link ServerSocketChannel} has its own boss thread. For example,
 ...
 * @apiviz.landmark
 */
public class NioServerSocketChannelFactory implements
	 ...
	 *            the maximum number of I/O worker threads
	 */
	public NioServerSocketChannelFactory(Executor bossExecutor, int bossCount,
			Executor workerExecutor, int workerCount) {
	...
	private static int getMaxThreads(Executor executor) {
		if (executor instanceof ThreadPoolExecutor) {
			final int maxThreads = ((ThreadPoolExecutor) executor)
					.getMaximumPoolSize();
