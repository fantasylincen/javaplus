...
final class LocalClientChannelSink extends AbstractChannelSink {
	...
	private static final InternalLogger logger = InternalLoggerFactory
			.getInstance(LocalClientChannelSink.class);
	...
	public void eventSunk(ChannelPipeline pipeline, ChannelEvent e)
		...
		if (e instanceof ChannelStateEvent) {
			...
			DefaultLocalChannel channel = (DefaultLocalChannel) event
					.getChannel();
	...
	private static void bind(DefaultLocalChannel channel, ChannelFuture future,
			LocalAddress localAddress) {
		try {
			if (!LocalChannelRegistry.register(localAddress, channel)) {
				throw new ChannelException("address already in use: "
						+ localAddress);
			}
	...
	private void connect(DefaultLocalChannel channel, ChannelFuture future,
		...
		try {
			pipeline = serverChannel.getConfig().getPipelineFactory()
					.getPipeline();
		} catch (Exception e) {
			future.setFailure(e);
		...
		DefaultLocalChannel acceptedChannel = new DefaultLocalChannel(
				serverChannel, serverChannel.getFactory(), pipeline, this,
				channel);
		channel.pairedChannel = acceptedChannel;

		// check if the channel was bound before. See #276
		if (!channel.isBound()) {
			bind(channel, succeededFuture(channel), new LocalAddress(
					LocalAddress.EPHEMERAL));
		}
