...
class OioDatagramPipelineSink extends AbstractOioChannelSink {
	...
	public void eventSunk(ChannelPipeline pipeline, ChannelEvent e)
		...
		if (e instanceof ChannelStateEvent) {
			...
			OioDatagramWorker.write(channel, future, evt.getMessage(),
					evt.getRemoteAddress());
		}
	}

	private void bind(OioDatagramChannel channel, ChannelFuture future,
		...
		try {
			...
			DeadLockProofWorker.start(workerExecutor,
					new ThreadRenamingRunnable(new OioDatagramWorker(channel),
	...
	private void connect(OioDatagramChannel channel, ChannelFuture future,
		...
		try {
			...
			if (!bound) {
				// Start the business.
				DeadLockProofWorker.start(workerExecutor,
						new ThreadRenamingRunnable(new OioDatagramWorker(
								channel), threadName, determiner));
			} else {
