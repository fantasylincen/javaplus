...
final class AtomicFieldUpdaterUtil {
	...
	static {
		...
		try {
			AtomicReferenceFieldUpdater<Node, Node> tmp = AtomicReferenceFieldUpdater
	...
	static <T, V> AtomicReferenceFieldUpdater<T, V> newRefUpdater(
			Class<T> tclass, Class<V> vclass, String fieldName) {
		if (AVAILABLE) {
			return AtomicReferenceFieldUpdater.newUpdater(tclass, vclass,
					fieldName);
		} else {
			return null;
		}
	...
	static <T> AtomicIntegerFieldUpdater<T> newIntUpdater(Class<T> tclass,
			String fieldName) {
		if (AVAILABLE) {
