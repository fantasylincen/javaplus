...
public abstract class FrameDecoder extends SimpleChannelUpstreamHandler
	...
	protected ChannelBuffer appendToCumulation(ChannelBuffer input) {
		...
		this.cumulation = input = ChannelBuffers.wrappedBuffer(cumulation,
				input);
		return input;
	}
	...
	protected ChannelBuffer updateCumulation(ChannelHandlerContext ctx,
		...
		if (readableBytes > 0) {
			...
			if (readableBytes < inputCapacity && inputCapacity > copyThreshold) {
				...
				cumulation = newCumulation = newCumulationBuffer(ctx,
						input.readableBytes());
	...
	protected final void unfoldAndFireMessageReceived(
			ChannelHandlerContext context, SocketAddress remoteAddress,
			Object result) {
		if (unfold) {
	...
	protected ChannelBuffer newCumulationBuffer(ChannelHandlerContext ctx,
			int minimumCapacity) {
		ChannelBufferFactory factory = ctx.getChannel().getConfig()
				.getBufferFactory();
		...
		try {
			if (cumulation != null) {
				Channels.fireMessageReceived(ctx,
						cumulation.readBytes(actualReadableBytes()));
			}
		} finally {
