...
abstract class AbstractNioWorker extends AbstractNioSelector implements Worker {
	...
	protected ThreadRenamingRunnable newThreadRenamingRunnable(int id,
		...
		return new ThreadRenamingRunnable(this, "New I/O worker #" + id,
				determiner);
	}
	...
	protected abstract boolean scheduleWriteIfNecessary(
	...
	protected void write0(AbstractNioChannel<?> channel) {
		...
		synchronized (channel.writeLock) {
			...
			for (;;) {
				...
				try {
					if (evt == null) {
						if ((channel.currentWriteEvent = evt = writeBuffer
								.poll()) == null) {
							removeOpWrite = true;
						...
						channel.currentWriteBuffer = buf = sendBufferPool
								.acquire(evt.getMessage());
					} else {
						future = evt.getFuture();
					...
					if (buf.finished()) {
						...
						if (writtenBytes > 0) {
							...
							future.setProgress(localWrittenBytes,
	...
	void setInterestOps(final AbstractNioChannel<?> channel,
			final ChannelFuture future, final int interestOps) {
		boolean iothread = isIoThread(channel);
		if (!iothread) {
		...
		try {
			...
			int newInterestOps = interestOps & ~Channel.OP_WRITE
					| channel.getRawInterestOps() & Channel.OP_WRITE;
