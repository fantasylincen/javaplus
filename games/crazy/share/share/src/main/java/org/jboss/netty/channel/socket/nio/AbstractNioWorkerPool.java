...
public abstract class AbstractNioWorkerPool<E extends AbstractNioWorker>
	...
	private static final InternalLogger logger = InternalLoggerFactory
			.getInstance(AbstractNioWorkerPool.class);
	...
	private void waitForWorkerThreads() {
		long deadline = System.nanoTime()
				+ TimeUnit.SECONDS.toNanos(INITIALIZATION_TIMEOUT);
		boolean warn = false;
		for (AbstractNioSelector worker : workers) {
			...
			try {
				...
				} else if (!worker.startupLatch.await(waitTime,
		...
		if (warn) {
			logger.warn("Failed to get all worker threads ready within "
	...
	/**
	 * Only here for backward compability and will be removed in later releases.
	 * Please use {@link #newWorker(Executor)}
	 * 
	 * 
	 * @param executor
	...
	protected E createWorker(Executor executor) {
		throw new IllegalStateException(
				"This will be removed. Override this and the newWorker(..) method!");
	}
	 ...
	 * service IO.
	 * 
	 * This method will be made abstract in further releases (once
	 * {@link #createWorker(Executor)} was removed).
	 * 
	 * 
	 * @param executor
	 *            the {@link Executor} to use
	...
	public E nextWorker() {
		return (E) workers[Math.abs(workerIndex.getAndIncrement()
				% workers.length)];
	}
