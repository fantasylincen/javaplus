...
public class ByteBufferBackedChannelBuffer extends AbstractChannelBuffer {
	...
	public void getBytes(int index, ByteBuffer dst) {
		...
		try {
			...
			throw new IndexOutOfBoundsException(
					"Too many bytes to read - Need " + (index + bytesToCopy)
							+ ", maximum is " + data.limit());
		}
		dst.put(data);
	}
	...
	public int getBytes(int index, GatheringByteChannel out, int length)
		...
		return out.write((ByteBuffer) buffer.duplicate().position(index)
				.limit(index + length));
	}
	...
	public int setBytes(int index, ScatteringByteChannel in, int length)
		...
		ByteBuffer slice = (ByteBuffer) buffer.duplicate()
				.limit(index + length).position(index);
		int readBytes = 0;
	...
	public ByteBuffer toByteBuffer(int index, int length) {
		if (index == 0 && length == capacity()) {
			...
			return ((ByteBuffer) buffer.duplicate().position(index)
					.limit(index + length)).slice().order(order());
		}
	}
	...
	public ChannelBuffer slice(int index, int length) {
		if (index == 0 && length == capacity()) {
			...
			return new ByteBufferBackedChannelBuffer(
					((ByteBuffer) buffer.duplicate().position(index)
