...
class OioServerSocketPipelineSink extends AbstractOioChannelSink {
	...
	static final InternalLogger logger = InternalLoggerFactory
			.getInstance(OioServerSocketPipelineSink.class);
	...
	private void handleServerSocket(ChannelEvent e) {
		...
		OioServerSocketChannel channel = (OioServerSocketChannel) event
				.getChannel();
	...
	private static void handleAcceptedSocket(ChannelEvent e) {
		if (e instanceof ChannelStateEvent) {
			ChannelStateEvent event = (ChannelStateEvent) e;
			OioAcceptedSocketChannel channel = (OioAcceptedSocketChannel) event
					.getChannel();
			ChannelFuture future = event.getFuture();
			ChannelState state = event.getState();
			Object value = event.getValue();
			...
			case CONNECTED:
				...
				break;
			case INTEREST_OPS:
				AbstractOioWorker.setInterestOps(channel, future,
	...
	private void bind(OioServerSocketChannel channel, ChannelFuture future,
		...
		try {
			...
			Executor bossExecutor = ((OioServerSocketChannelFactory) channel
					.getFactory()).bossExecutor;
			DeadLockProofWorker.start(bossExecutor, new ThreadRenamingRunnable(
					new Boss(channel), "Old I/O server boss (" + channel + ')',
					determiner));
			bossStarted = true;
		} catch (Throwable t) {
			future.setFailure(t);
	...
	private static void close(OioServerSocketChannel channel,
			...
			try {
				while (channel.isBound()) {
					try {
						...
						try {
							ChannelPipeline pipeline = channel.getConfig()
									.getPipelineFactory().getPipeline();
							final OioAcceptedSocketChannel acceptedChannel = new OioAcceptedSocketChannel(
