...
import cn.javaplus.shhz.exceptions.ShhzException;
import cn.javaplus.shhz.util.StringResolver;
...
class PlistImpl implements Plist {
	...
	PlistImpl(String filePath) {
		...
		try {
			...
			throw new ShhzException("plist file error:" + filePath);
		}
	}
	 ...
	 * （非 Javadoc）
	 * 
	 * @see cn.javaplus.game.shhz.plist.Plist#getFrames()
	 */
	@Override
	...
	private FrameImpl buildFrame(Element key, Element value) {
		...
		FrameRect rect = buildRect(value.getChild(1).getText());
