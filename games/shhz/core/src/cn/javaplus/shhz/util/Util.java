...
import cn.javaplus.shhz.random.IntegerFetcher;
...
public class Util {
	...
	public static final class Str {
		...
		// System.out.println(appendFilePath("aa/bbb/cccc/d", "aa/bbb/cccc/d"));
		// System.out.println(appendFilePath("aa/bbb/cccc/d",
		// "aa/bbb/cccc/d/"));
		// System.out.println(appendFilePath("aa/bbb/cccc/d",
		// "/aa/bbb/cccc/d"));
		// System.out.println(appendFilePath("aa/bbb/cccc/d",
		...
		//
		//
		// System.out.println(appendFilePath("D:/aa/bbb/cccc/d",
		// "aa/bbb/cccc/d"));
		// System.out.println(appendFilePath("D:/aa/bbb/cccc/d",
		// "aa/bbb/cccc/d/"));
		// System.out.println(appendFilePath("D:/aa/bbb/cccc/d",
		...
		// public static void main(String[] args) {
		// for (int i = 0; i < 100; i++) {
		// String ensureMaxLen = Str.ensureMaxLen("烂赛单反是扽sadf 阿萨德发速度", i);
		// System.out.println(ensureMaxLen);
		// System.out.println(ensureMaxLen.getBytes().length);
		// }
		//
		// }
	...
	public static final class Collection {
		...
		public static void main(String[] args) {
			List<Integer> ls = getIntegers("10-14:1,15-19:2,20-24:3,25-29:4,30-100:5");
	...
	public static final class Shape {
		...
		public static final class Rhombus {
			...
			/**
			 * point pointInRhombus is in rhombus
			 * 
			 * @param pointInRhombus
			 * @param rhombus
			 * @return
			 */
			public static boolean isIn(Vector2 pointInRhombus,
					cn.javaplus.shhz.shape.Rhombus rhombus) {

				float h = rhombus.getW();
				float w = rhombus.getH();
				float x1 = rhombus.getX() + h / 2;
				float y1 = rhombus.getY() + w / 2;

				return isIn(pointInRhombus, h, w, x1, y1);
			}
			...
			private static boolean isIn(Vector2 pointInRhombus, float h,
					float w, float x1, float y1) {
				float x0 = pointInRhombus.x;
				float y0 = pointInRhombus.y;

				return isIn(h, w, x1, y1, x0, y0);
			}

			private static boolean isIn(float h, float w, float x1, float y1,
					float x0, float y0) {
				float x2 = x0 - x1;
				float y2 = y0 - y1;
				...
				float a = y2 - k * x2;
				float b = y2 + k * x2;
				return -w / 2 < a && a < w / 2 && -w / 2 < b && b < w / 2;
			}

			public static boolean isIn(int x0, int y0, RhombusFloat rhombus) {
				float h = rhombus.getW();
				float w = rhombus.getH();
				float x1 = rhombus.getX() + h / 2;
				float y1 = rhombus.getY() + w / 2;
		...
		public static final class Polygon {
			...
			public static final boolean isPointInPolygon(Vector2 point,
					cn.javaplus.shhz.shape.Polygon polygon) {
			...
			private static boolean isPointInPolygon(float x, float y,
					List<Vector2> polygon) {
				Vector2 lastVertice = polygon.get(polygon.size() - 1);
				boolean oddNodes = false;
				for (int i = 0; i < polygon.size(); i++) {
					Vector2 vertice = polygon.get(i);
					if ((vertice.y < y && lastVertice.y >= y)
							|| (lastVertice.y < y && vertice.y >= y)) {
						if (vertice.x + (y - vertice.y)
								/ (lastVertice.y - vertice.y)
								* (lastVertice.x - vertice.x) < x) {
							oddNodes = !oddNodes;
						}
					}
					lastVertice = vertice;
