...
import org.javaplus.game.common.json.JsonReader;
...
public class BodyEditorLoader {
	...
	public void attachFixture(Body body, String name, FixtureDef fd, float scale) {
		...
		Vector2 origin = mul(vec.set(rbModel.origin),scale);
		...
		for (int i=0, n=rbModel.polygons.size(); i<n; i++) {
			...
			for (int ii=0, nn=vertices.length; ii<nn; ii++) {
				vertices[ii] = mul(newVec().set(polygon.vertices.get(ii)), scale);
		...
		for (int i=0, n=rbModel.circles.size(); i<n; i++) {
			...
			Vector2 center = mul(newVec().set(circle.center),scale);
	...
	private Vector2 mul(Vector2 set, float scale) {
		set.set(set.x * scale, set.y * scale);
		return set;
	}
	...
	public Vector2 getOrigin(String name, float scale) {
		...
		return mul(vec.set(rbModel.origin), scale);
	}
