...
b2ContactFilter defaultFilter;
	...
	*/
	...
	public IBody createBody (BodyDef def) {
		long bodyAddr = jniCreateBody(addr, def.getType().getValue(), def.getPosition().getX(), def.getPosition().getY(), def.getAngle(), def.getLinearVelocity().getX(),
			def.getLinearVelocity().getY(), def.getAngularVelocity(), def.getLinearDamping(), def.getAngularDamping(), def.isAllowSleep(), def.isAwake(),
			def.isFixedRotation(), def.isBullet(), def.isActive(), def.getGravityScale());
	...
	private long createProperJoint (JointDef def) {
		...
		if (def.type == JointType.WeldJoint) {
			...
			return jniCreateWeldJoint(addr, d.bodyA.addr, d.bodyB.addr, d.collideConnected, d.localAnchorA.getX(), d.localAnchorA.getY(),
				d.localAnchorB.getX(), d.localAnchorB.getY(), d.referenceAngle);
		}
...
// b2Contact* GetContactList();
	...
	*/
	...
	public void rayCast (RayCastCallback callback, Vector2 point1, Vector2 point2) {
		...
		jniRayCast(addr, point1.getX(), point1.getY(), point2.getX(), point2.getY());
	}
