...
public class PixmapPacker implements Disposable {
	...
	public synchronized IRectangle pack (String name, Pixmap image) {
	...
	public synchronized IRectangle getRect (String name) {
		for (Page page : pages) {
			IRectangle rect = page.rects.get(name);
	...
	public synchronized int getPageIndex (String name) {
		for (int i = 0; i < pages.size; i++) {
			IRectangle rect = pages.get(i).rects.get(name);
