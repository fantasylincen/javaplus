...
public class AssetManager implements Disposable {
	...
	public AssetManager(FileHandleResolver resolver) {
		...
		setLoader(Model.class, ".g3dj", new G3dModelLoader(new JsonReader(),
				resolver));
		setLoader(Model.class, ".g3db", new G3dModelLoader(new UBJsonReader(),
				resolver));
	...
	public synchronized <T> boolean containsAsset(T asset) {
		ObjectMap<String, RefCountedContainer> typedAssets = assets.get(asset
				.getClass());
		if (typedAssets == null)
			return false;
		for (String fileName : typedAssets.keys()) {
			T otherAsset = (T) typedAssets.get(fileName)
	...
	public synchronized <T> String getAssetFileName(T asset) {
		for (Class assetType : assets.keys()) {
			ObjectMap<String, RefCountedContainer> typedAssets = assets
					.get(assetType);
			for (String fileName : typedAssets.keys()) {
				T otherAsset = (T) typedAssets.get(fileName).getObject(
						Object.class);
				if (otherAsset == asset || asset.equals(otherAsset))
					return fileName;
			}
	...
	public synchronized <T> void load(String fileName, Class<T> type,
		...
		if (loader == null)
			throw new GdxRuntimeException("No loader for type: "
		...
		toLoad++;
		AssetDescriptor assetDesc = new AssetDescriptor(fileName, type,
				parameter);
	...
	/**
	 * Disposes the given asset and all its dependencies recursively, depth
	 * first.
	 * 
	 * @param fileName
	 */
	 ...
	 * milliseconds, yeilding the CPU to the loading thread between updates.
	...
	private synchronized void injectDependency(String parentAssetFilename,
		...
		if (isLoaded(dependendAssetDesc.fileName)) {
			...
			RefCountedContainer assetRef = assets.get(type).get(
					dependendAssetDesc.fileName);
	...
	private boolean updateTask() {
		...
		if (task.update()) {
			addAsset(task.assetDesc.fileName, task.assetDesc.type,
					task.getAsset());
	...
	public synchronized <T, P extends AssetLoaderParameters<T>> void setLoader(
		...
		if (loaders == null)
			this.loaders.put(type,
					loaders = new ObjectMap<String, AssetLoader>());
	...
	/**
	 * @return a string containg ref count and dependency information for all
	 *         assets.
	 */
