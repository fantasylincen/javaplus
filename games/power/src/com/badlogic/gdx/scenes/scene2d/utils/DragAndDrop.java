...
public class DragAndDrop {
	...
	public void addSource (final Source source) {
		DragListener listener = new DragListener() {
			...
			public void drag (InputEvent event, float x, float y, int pointer) {
				...
				isValidTarget = false;
				IActor hit = event.getStage().hit(event.getStageX(), event.getStageY(), true); // Prefer touchable actors.
				...
				if (hit != null) {
					for (int i = 0, n = targets.size; i < n; i++) {
						...
						isValidTarget = target.drag(source, payload, tmpVector.getX(), tmpVector.getY(), pointer);
						break;
					}
				}
				if (newTarget != target) {
	...
	static public class Payload {
		...
		public IActor getValidDragActor () {
		...
		public IActor getInvalidDragActor () {
