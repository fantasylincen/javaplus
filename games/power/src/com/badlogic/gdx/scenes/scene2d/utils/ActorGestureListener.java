 ...
 * @see GestureDetector
 * @author Nathan Sweet */
public abstract class ActorGestureListener implements EventListener {
	static final Vector2 tmpCoords = new Vector2();

	private final GestureDetector detector;
	InputEvent event;
	IActor actor, touchDownTarget;
	...
	protected abstract void touchDown (InputEvent event, float x, float y, int pointer, int button);

	protected abstract void touchUp (InputEvent event, float x, float y, int pointer, int button);

	protected abstract void tap (InputEvent event, float x, float y, int count, int button);
	...
	protected abstract boolean longPress (IActor actor, float x, float y);

	protected abstract void fling (InputEvent event, float velocityX, float velocityY, int button);

	/** The delta is the difference in stage coordinates since the last pan. */
	protected abstract void pan (InputEvent event, float x, float y, float deltaX, float deltaY);

	protected abstract void zoom (InputEvent event, float initialDistance, float distance);

	protected abstract void pinch (InputEvent event, Vector2 initialPointer1, Vector2 initialPointer2, Vector2 pointer1, Vector2 pointer2);

	public GestureDetector getGestureDetector () {
		return detector;
	}

	public IActor getTouchDownTarget () {
