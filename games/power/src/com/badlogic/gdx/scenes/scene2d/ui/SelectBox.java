...
public class SelectBox extends Widget implements Disableable {
	...
	class SelectList extends ScrollPane {
		...
		public void show (IStage stage) {
			...
			float heightBelow = tmpCoords.getY();
			float heightAbove = stage.getCamera().getViewPortHeight() - tmpCoords.getY() - SelectBox.this.getHeight();
			boolean below = true;
			...
			if (below)
				setY(tmpCoords.getY() - height);
			else
				setY(tmpCoords.getY() + SelectBox.this.getHeight());
			setX(tmpCoords.getX());
		...
		public IActor hit (float x, float y, boolean touchable) {
			IActor actor = super.hit(x, y, touchable);
		...
		public void act (float delta) {
			super.act(delta);
			SelectBox.this.localToStageCoordinates(tmpCoords.set(0, 0));
			if (tmpCoords.getX() != screenCoords.getX() || tmpCoords.getY() != screenCoords.getY()) hideList();
		}
	}
