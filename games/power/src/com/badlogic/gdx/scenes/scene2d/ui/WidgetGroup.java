...
public class WidgetGroup extends Group implements Layout {
	...
	private void setLayoutEnabled (IGroup parent, boolean enabled) {
		SnapshotArray<IActor> children = getChildren();
		for (int i = 0, n = children.size; i < n; i++) {
			IActor actor = children.get(i);
			if (actor instanceof Layout)
				((Layout)actor).setLayoutEnabled(enabled);
			else if (actor instanceof IGroup) //
				setLayoutEnabled((IGroup)actor, enabled);
		}
	}
