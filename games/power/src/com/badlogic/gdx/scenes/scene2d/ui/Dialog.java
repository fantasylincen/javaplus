...
public class Dialog extends Window {
	...
	private Skin skin;
	ObjectMap<IActor, Object> values = new ObjectMap();
	boolean cancelHide;
	IActor previousKeyboardFocus, previousScrollFocus;
	...
	private void initialize () {
		...
		buttonTable.addListener(new ChangeListener() {
			public void changed (ChangeEvent event, IActor actor) {
		...
		addListener(new FocusListener() {
			public void keyboardFocusChanged (FocusEvent event, IActor actor, boolean focused) {
				if (!focused) focusChanged(event);
			}

			public void scrollFocusChanged (FocusEvent event, IActor actor, boolean focused) {
				if (!focused) focusChanged(event);
			}
			...
			private void focusChanged (FocusEvent event) {
				...
				if (isModal && stage != null && stage.getRoot().getChildren().size > 0
					...
					IActor newFocusedActor = event.getRelatedActor();
	...
	public Dialog show (IStage stage) {
		clearActions();
		removeCaptureListener(ignoreTouchDown);

		previousKeyboardFocus = null;
		IActor actor = stage.getKeyboardFocus();
