...
public final class Intersector {
	...
	public static int pointLineSide (Vector2 linePoint1, Vector2 linePoint2, Vector2 point) {
		return (int)Math.signum((linePoint2.getX() - linePoint1.getX()) * (point.getY() - linePoint1.getY()) - (linePoint2.getY() - linePoint1.getY())
	...
	public static float distanceLinePoint (Vector2 start, Vector2 end, Vector2 point) {
		tmp.set(end.getX(), end.getY(), 0);
		float l2 = tmp.sub(start.getX(), start.getY(), 0).len2();
		if (l2 == 0.0f) // start == end
			return point.dst(start);

		tmp.set(point.getX(), point.getY(), 0);
		tmp.sub(start.getX(), start.getY(), 0);
		tmp2.set(end.getX(), end.getY(), 0);
		tmp2.sub(start.getX(), start.getY(), 0);
		...
		tmp.set(end.getX(), end.getY(), 0); // Projection falls on the segment
		tmp.sub(start.getX(), start.getY(), 0).scl(t).add(start.getX(), start.getY(), 0);
		return tmp2.set(point.getX(), point.getY(), 0).dst(tmp);
	}
	...
	public static boolean intersectSegmentCircle (Vector2 start, Vector2 end, Vector2 center, float squareRadius) {
		...
		if (u <= 0) {
			...
			tmp2.set(tmp3.x + start.getX(), tmp3.y + start.getY(), 0);
		}

		float x = center.getX() - tmp2.x;
		float y = center.getY() - tmp2.y;
