...
import cn.javaplus.twolegs.tests.utils.PerspectiveCamController;
...
public class ProjectiveTextureTest extends GdxTest {
	...
	public void setupScene() {
		plane = new Mesh(true, 4, 6, new VertexAttribute(Usage.Position, 3,
				ShaderProgram.POSITION_ATTRIBUTE), new VertexAttribute(
				Usage.Normal, 3, ShaderProgram.NORMAL_ATTRIBUTE));
		plane.setVertices(new float[] { -10, -1, 10, 0, 1, 0, 10, -1, 10, 0, 1,
				0, 10, -1, -10, 0, 1, 0, -10, -1, -10, 0, 1, 0 });
		...
		projector = new PerspectiveCamera(67, Gdx.graphics.getWidth(),
				Gdx.graphics.getHeight());
	...
	public void setupUI() {
		...
		TextButton reload = new TextButton("Reload Shaders",
		...
		reload.addListener(new ClickListener() {
			public void clicked(InputEvent event, float x, float y) {
				ShaderProgram prog = new ShaderProgram(Gdx.files.internal(
						"data/shaders/projtex-vert.glsl").readString(),
						Gdx.files.internal("data/shaders/projtex-frag.glsl")
								.readString());
	...
	public void setupShaders() {
		...
		projTexShader = new ShaderProgram(Gdx.files.internal(
				"data/shaders/projtex-vert.glsl").readString(), Gdx.files
	...
	public void render() {
		...
		if (camera.getSelectedIndex() == 0) {
			renderMesh(projTexShader, cam.combined, projector.combined,
					planeTrans, plane, Color.WHITE);
			/*
			 * TODO: Fix method rendering renderMesh(projTexShader,
			 * cam.combined, projector.combined, cubeTrans, cube, Color.WHITE);
			 */
		} else {
			renderMesh(projTexShader, projector.combined, projector.combined,
					planeTrans, plane, Color.WHITE);
			/*
			 * TODO: Fix method rendering renderMesh(projTexShader,
			 * projector.combined, projector.combined, cubeTrans, cube,
			 * Color.WHITE);
			 */
		}
	...
	private void renderMesh(ShaderProgram shader, Matrix4 cam,
			Matrix4 projector, Matrix4 model, Mesh mesh, Color color) {
