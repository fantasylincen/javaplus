...
public class FilesTest extends GdxTest {
	...
	public void create() {
		...
		if (Gdx.files.isExternalStorageAvailable()) {
			...
			try {
				out = new BufferedWriter(new OutputStreamWriter(Gdx.files
						.external("test.txt").write(false)));
				out.write("test");
			...
			try {
				in = new BufferedReader(new InputStreamReader(Gdx.files
						.external("test.txt").read()));
		...
		if (Gdx.files.isLocalStorageAvailable()) {
			...
			BufferedReader in = null;
			try {
				in = new BufferedReader(new InputStreamReader(Gdx.files.local(
						"test.txt").read()));
				if (!in.readLine().equals("test"))
					message += "Read result wrong\n";
				else
	...
	private void testClasspath() throws IOException {
		...
		FileHandle handle = Gdx.files
				.classpath("com/badlogic/gdx/utils/arial-15.png");
		if (!handle.exists())
			fail();
	...
	private void testAbsolute() throws IOException {
		String path = new File(Gdx.files.getExternalStoragePath(), "meow")
				.getAbsolutePath();
