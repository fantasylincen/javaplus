...
public class Box2DTest extends GdxTest implements InputProcessor {
	...
	public void create() {
		...
		textureRegion = new TextureRegion(new Texture(
				Gdx.files.internal("data/badlogicsmall.jpg")));
	...
	private void createPhysicsWorld() {
		...
		world.setContactListener(new ContactListener() {
			...
			public void postSolve(Contact contact, ContactImpulse impulse) {
				...
				// System.out.println("post solve, normal impulses: " + ni[0] +
				// ", " + ni[1] + ", tangent impulses: " + ti[0] + ", " +
				// ti[1]);
			}
		});
	}
	...
	public void render() {
		...
		for (int i = 0; i < boxes.size(); i++) {
			...
			Vector2 position = box.getPosition(); // that's the box's center
													// position
			float angle = MathUtils.radiansToDegrees * box.getAngle(); // the
																		// rotation
																		// angle
																		// around
																		// the
																		// center
			batch.draw(textureRegion, position.x - 1, position.y - 1, // the
																		// bottom
																		// left
																		// corner
																		// of
