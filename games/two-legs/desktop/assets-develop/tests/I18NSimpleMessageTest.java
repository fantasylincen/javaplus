...
/**
 * Performs some tests with {@link I18NBundle} and prints the results on the
 * screen.
 * 
 * @author davebaol
 */
public class I18NSimpleMessageTest extends GdxTest {
	...
	public void create() {
		...
		try {
			...
			rb_unsupported = I18NBundle.createBundle(bfh, new Locale(
					"unsupported"));
			...
			message += t.getClass();
			Gdx.app.error(I18NSimpleMessageTest.class.getSimpleName(), "Error",
					t);
		}
	}

	private String getMessage(String header, I18NBundle rb) {
		return header + " -> locale: " + rb.getLocale() + ", msg: \""
				+ rb.format("msg") + "\", rootMsg: \"" + rb.format("rootMsg")
				+ "\"";
	}

	private String getParametricMessage(String header, I18NBundle rb) {
		return header + " -> "
				+ rb.format("msgWithArgs", "LibGDX", MathUtils.PI, now);
	}
