...
public class AssetManagerTest extends GdxTest implements AssetErrorListener {
	...
	private void load() {
		...
		// map =
		// TiledLoader.createMap(Gdx.files.internal("data/tiledmap/tilemap csv.tmx"));
		// atlas = new TileAtlas(map, Gdx.files.internal("data/tiledmap/"));
		// renderer = new TileMapRenderer(map, atlas, 8, 8);
		System.out.println("plain took: " + (TimeUtils.nanoTime() - start)
				/ 1000000000.0f);
		...
		// manager.load("data/tiledmap/tilemap csv.tmx", TileMapRenderer.class,
		// new
	...
	public void render() {
		...
		if (result) {
			if (!diagnosed) {
				diagnosed = true;
				System.out.println("took: " + (TimeUtils.nanoTime() - start)
						/ 1000000000.0f);
				elapsed = 0;
			} else {
		...
		if (manager.isLoaded("data/verdana39.png"))
			batch.draw(manager.get("data/verdana39.png", Texture.class), 300,
					100);
		...
		if (manager.isLoaded("data/verdana39.fnt"))
			manager.get("data/verdana39.fnt", BitmapFont.class).draw(batch,
					"This is a test", 100, 80);
		if (manager.isLoaded("data/multipagefont.fnt"))
			manager.get("data/multipagefont.fnt", BitmapFont.class).draw(batch,
					"This is a test qpRPN multi page!", 100, 80);

		// System.out.println(Arrays.toString(manager.getAssetNames().items));

		// if (manager.isLoaded("data/test.etc1"))
		// batch.draw(manager.get("data/test.etc1", Texture.class), 0, 0);
		// if (manager.isLoaded("data/tiledmap/tilemap csv.tmx"))
		// manager.get("data/tiledmap/tilemap csv.tmx",
		// TileMapRenderer.class).render();
		if (manager.isLoaded("data/i18n/message2"))
			font.draw(batch, manager
					.get("data/i18n/message2", I18NBundle.class).get("msg"),
					100, 400);

		font.draw(batch, "loaded: " + manager.getProgress() + ", reloads: "
				+ reloads, 0, 30);
		batch.end();
	...
	public void error(AssetDescriptor asset, Throwable throwable) {
		Gdx.app.error("AssetManagerTest", "Couldn't load asset: " + asset,
				(Exception) throwable);
	}
