...
package cn.javaplus.twolegs.tests.net;
...
public class NetAPITest extends GdxTest implements HttpResponseListener {
	...
	public void create() {
...

		{
			...
			statusLabel.setPosition(Gdx.graphics.getWidth() * 0.5f
					- statusLabel.getWidth() * 0.5f, 30f);
...

		{
			ClickListener clickListener = new ClickListener() {
				...
				public void clicked(InputEvent event, float x, float y) {
					...
					statusLabel.setText("Downloading data from "
							+ httpRequest.getUrl());
				}
			};
			...
			ClickListener cancelListener = new ClickListener() {
				...
				public void clicked(InputEvent event, float x, float y) {
					...
					if (httpRequest != null) {
						...
						Gdx.app.log("NetAPITest", "Cancelling request "
								+ httpRequest.getUrl());
						statusLabel.setText("Cancelling request "
								+ httpRequest.getUrl());
					}
				}
			};
	...
	public void handleHttpResponse(HttpResponse httpResponse) {
		...
		// We are not in main thread right now so we need to post to main thread
		// for ui updates
		...
		if (clickedButton == btnDownloadImage) {
			...
			Gdx.app.postRunnable(new Runnable() {
				public void run() {
					Pixmap pixmap = new Pixmap(rawImageBytes, 0,
	...
	public void failed(Throwable t) {
		setButtonDisabled(false);
		statusLabel.setText("Failed to perform the HTTP Request: "
				+ t.getMessage());
		t.printStackTrace();
	}
	...
	public void render() {
		...
		if (texture != null) {
			batch.begin();
			batch.draw(texture,
					Gdx.graphics.getWidth() * 0.5f - texture.getWidth() * 0.5f,
					100f);
			batch.end();
		} else if (text != null) {
			batch.begin();
