...
import cn.javaplus.twolegs.tests.utils.OrthoCamController;
...
/**
 * This is a simple demonstration of how to perform VERY basic culling on
 * hierarchies of stage actors that do not scale or rotate. It is not a general
 * solution as it assumes that actors and groups are only translated (moved,
 * change their x/y coordinates). NOTE: This has been obsoleted by
 * {@link Cullable}.
 * 
 * @author mzechner
 */
public class SimpleStageCullingTest extends GdxTest {
	...
	/**
	 * We need to extend a base actor class so we can add the culling in the
	 * render method. We also add a method to get the stage coordinates of the
	 * actor so we can cull it against the camera's view volume.
	 * 
	 * @author mzechner
	 */
	private class CullableActor extends Image {
		...
		public CullableActor(String name, Texture texture,
		...
		public void draw(Batch batch, float parentAlpha) {
			// if this actor is not within the view of the camera we don't draw
			// it.
			if (isCulled())
				return;
	...
	public void create() {
		...
		stage = new Stage();
		;
		camController = new OrthoCamController(
				(OrthographicCamera) stage.getCamera()); // we know it's an
															// ortho cam at this
															// point!
		...
		for (int i = 0; i < 5000; i++) {
			Actor img = new CullableActor("img" + i, texture,
		...
		// we also want to output the number of visible actors, so we need a
		// SpriteBatch and a BitmapFont
	...
	public void render() {
		...
		font.draw(
				batch,
				"Visible: " + numVisible + ", fps: "
						+ Gdx.graphics.getFramesPerSecond(), 20, 30);
		batch.end();
	}
