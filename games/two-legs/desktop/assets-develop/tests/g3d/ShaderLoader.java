...
public class ShaderLoader {
	...
	protected void load(final StringBuilder out, final String name) {
		...
		final String snipName = idx < 0 || (idx >= name.length() - 1) ? ""
		...
		if (result == null)
			throw new GdxRuntimeException("No snippet [" + snipName
					+ "] in file " + root.child(fileName).path());
		parse(out, fileName, result);
	}

	protected void parse(final StringBuilder out, final String currentFile,
			final String code) {
	...
	public String toString() {
		stringBuilder.setLength(0);
		for (final ObjectMap.Entry<String, ObjectMap<String, String>> entry : snippets
				.entries()) {
