...
public class Benchmark3DTest extends BaseG3dHudTest {
	...
	protected Label vertexCountLabel, textureBindsLabel, shaderSwitchesLabel,
			drawCallsLabel, glCallsLabel, lightsLabel;
	...
	public void create() {
		...
		lightingCheckBox.setPosition(hudWidth - lightingCheckBox.getWidth(),
				gridCheckBox.getTop());
		...
		lightsCheckBox.setPosition(hudWidth - lightsCheckBox.getWidth(),
				lightingCheckBox.getTop());
	...
	protected void randomizeLights() {
		...
		for (int i = 0; i < pointLights; i++) {
			environment.add(new PointLight().set(randomColor(),
					randomPosition(), MathUtils.random(10f)));
		}

		for (int i = 0; i < directionalLights; i++) {
			environment.add(new DirectionalLight().set(randomColor(),
					randomPosition()));
		}
	}

	protected Color randomColor() {
		return new Color(MathUtils.random(1.0f), MathUtils.random(1.0f),
				MathUtils.random(1.0f), MathUtils.random(1.0f));
	}

	protected Vector3 randomPosition() {
		return new Vector3(MathUtils.random(-10f, 10f), MathUtils.random(-10f,
				10f), MathUtils.random(-10f, 10f));
	}
