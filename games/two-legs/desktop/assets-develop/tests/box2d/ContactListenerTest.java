...
package cn.javaplus.twolegs.tests.box2d;
...
public class ContactListenerTest extends Box2DTest implements ContactListener {
	...
	protected void createWorld(World world) {
...

		{
			...
			for (int i = 0; i < 6; i++) {
				vertices[i] = new Vector2(0.5f * (float) Math.cos(angle),
						0.5f * (float) Math.sin(angle));
				angle += delta;
			}
	...
	public void beginContact(Contact contact) {
		System.out.println(String.format("beginContact() addr=%d",
				getContactAddr(contact)));
		...
		final Body toRemove = contact.getFixtureA().getBody().getType() == BodyType.DynamicBody ? contact
				.getFixtureA().getBody() : contact.getFixtureB().getBody();
	...
	public void endContact(Contact contact) {
		System.out.println(String.format("  endContact() addr=%d",
				getContactAddr(contact)));
	...
	public long getFixtureAddrA(Contact contact) {
		try {
			...
			Method getFixtureA = contact.getClass().getDeclaredMethod(
					"jniGetFixtureA", long.class);
	...
	public long getFixtureAddrB(Contact contact) {
		try {
			...
			Method getFixtureB = contact.getClass().getDeclaredMethod(
					"jniGetFixtureB", long.class);
